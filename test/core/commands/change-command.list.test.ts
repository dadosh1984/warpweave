...
describe('ChangeCommand.list with a change that has no proposal.md', () => {
  ...
  beforeAll(async () => {
    ...
    // What `warpweave new change` leaves behind, plus tasks: no proposal.md.
    const scaffolded = path.join(tempRoot, 'openspec', 'changes', 'scaffolded');
    await fs.mkdir(scaffolded, { recursive: true });
    await fs.writeFile(path.join(scaffolded, '.warpweave.yaml'), 'schema: spec-driven\n', 'utf-8');
  ...
  it('lists it, matching what `warpweave list` resolves', async () => {
