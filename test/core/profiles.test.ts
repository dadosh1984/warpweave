...
describe('profiles', () => {
  describe('CORE_WORKFLOWS', () => {
    it('should contain the default core workflows', () => {
      expect(CORE_WORKFLOWS).toEqual(['propose', 'explore', 'apply', 'update', 'sync', 'archive', 'translator', 'ponytail-minimal-output', 'superpowers-tdd', 'security-scan', 'drift-check', 'guardrails']);
    });
  ...
  describe('ALL_WORKFLOWS', () => {
    it('should contain all 27 workflows', () => {
      expect(ALL_WORKFLOWS).toHaveLength(27);
    });
    ...
    it('should contain expected workflow IDs', () => {
      const expected = [
        ...
        'ponytail-minimal-output', 'superpowers-tdd', 'security-scan', 'drift-check', 'release-compare',
      ];
