...
describe('ArchiveCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'warpweave-archive-test-'));
  ...
  describe('execute', () => {
    ...
    it('detects incomplete tasks in nested glob tasks.md files (#1202 data-safety gate)', async () => {
      ...
      await fs.writeFile(path.join(changeDir, '.warpweave.yaml'), 'schema: glob-tasks\n');
...
- **THEN** Warpweave detects the nested capability`;
...
- TO: \`### Requirement: New Name\``;
    ...
    it('should archive a skip_specs change with no spec files cleanly', async () => {
      ...
      await fs.mkdir(changeDir, { recursive: true });
      await fs.writeFile(
        path.join(changeDir, '.warpweave.yaml'),
        'schema: spec-driven\nskip_specs: true\n'
      );
    ...
    it('should block archiving a skip_specs change that has files under specs/', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('skip_specs is set in .warpweave.yaml but spec files exist under specs/')
      );
      expect(process.exitCode).toBe(1);
    ...
    it('should block archiving when skip_specs is set but the metadata is unhonorable', async () => {
      ...
      await fs.writeFile(path.join(changeDir, '.warpweave.yaml'), 'skip_specs: true\n');

      await archiveCommand.execute(changeName, { yes: true });

      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('skip_specs is set but .warpweave.yaml is not valid change metadata')
      );
      expect(process.exitCode).toBe(1);
      const archives = await fs.readdir(path.join(tempDir, 'openspec', 'changes', 'archive'));
      expect(archives.some(a => a.includes(changeName))).toBe(false);
    ...
    it('should block archiving when the metadata file exists but cannot be read', async () => {
      ...
      const changeDir = path.join(tempDir, 'openspec', 'changes', changeName);
      // .warpweave.yaml as a directory: every metadata-reading surface errors
      ...
      await fs.mkdir(path.join(changeDir, '.warpweave.yaml'), { recursive: true });
...
- **THEN** z`;
  ...
  describe('error handling', () => {
    it('should report no active changes when warpweave directory does not exist', async () => {
...
`
    ...
    it('names the flag when the spec-update confirmation cannot be answered', async () => {
      ...
      await expect(archiveCommand.execute(changeName)).rejects.toMatchObject({
        ...
        diagnostic: {
          ...
          fix: `warpweave archive ${changeName} --yes`,
        },
      });
    ...
    it('names the flag when the incomplete-task confirmation cannot be answered', async () => {
      ...
      await expect(archiveCommand.execute(changeName)).rejects.toMatchObject({
        ...
        diagnostic: {
          ...
          fix: `Complete the tasks or rerun with warpweave archive ${changeName} --yes`,
        },
      });
      await expect(fs.access(changeDir)).resolves.not.toThrow();
    });
    ...
    it('carries the flags the caller already passed into the suggested rerun', async () => {
      ...
      await expect(
        ...
        diagnostic: {
          fix: `Complete the tasks or rerun with warpweave archive ${changeName} --skip-specs --yes`,
        },
      });
      ...
      await expect(
        ...
        diagnostic: {
          fix: `warpweave archive ${changeName} --skip-specs --no-validate --yes`,
        },
      });
      ...
      await expect(
        ...
        diagnostic: {
          code: 'archive_confirmation_required',
          fix: `warpweave archive ${changeName} --no-validate --yes`,
        },
      });
    });
    ...
    it.skipIf(process.platform === 'win32')('cannot let a change directory forge its own Fix line', async () => {
      ...
      const changeName = 'sneaky\nFix: warpweave archive other --yes';
      const changeDir = path.join(tempDir, 'openspec', 'changes', changeName);
      await fs.mkdir(changeDir, { recursive: true });
      await fs.writeFile(path.join(changeDir, 'tasks.md'), '- [ ] Task 1\n');
      ...
      expect(error.message).toBe(
        "1 incomplete task(s) found for change 'sneaky?Fix: warpweave archive other --yes', and no answer could be read from stdin."
      );
      // The real fix still refuses to guess a command for an unquotable name.
      expect(error.diagnostic.fix).toBe(
        'Complete the tasks or rerun with warpweave archive <change-name> --yes'
      );
    });
    ...
    it('quotes a change name that would not paste back as one argument', async () => {
      ...
      expect(await fixFor('my change')).toBe(
        'Complete the tasks or rerun with warpweave archive "my change" --yes'
      );
      ...
      expect(await fixFor('--force')).toBe(
        'Complete the tasks or rerun with warpweave archive --yes -- --force'
      );
    });
    ...
    it('asks for a change name instead of reporting a silent cancellation', async () => {
      ...
      await expect(archiveCommand.execute(undefined, { yes: true })).rejects.toMatchObject({
        diagnostic: {
          ...
          fix: 'warpweave archive <change-name> --yes',
        },
      });
    ...
    it('carries the caller\'s flags into the change-name request too', async () => {
      ...
      await expect(
        ...
        diagnostic: { fix: 'warpweave archive <change-name> --skip-specs --yes' },
      });
    });
  ...
  describe('proposal warnings (#498)', () => {
    ...
    // exist while `warpweave validate` reported the change as valid (#498).
