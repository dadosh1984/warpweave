...
describe('UpdateCommand', () => {
  ...
  describe('basic validation', () => {
    it('should throw error if warpweave directory does not exist', async () => {
      // Remove warpweave directory
      ...
      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No Warpweave directory found. Run 'warpweave init' first."
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: warpweave-explore');
    ...
    it('should show the Hermes setup note when updating a configured Hermes tool', async () => {
      const exploreSkillDir = path.join(testDir, '.hermes', 'skills', 'warpweave-explore');
      await fs.mkdir(exploreSkillDir, { recursive: true });
      await fs.writeFile(
        path.join(exploreSkillDir, 'SKILL.md'),
    ...
    it('should migrate Warpweave skills from legacy .kimi to .kimi-code, preserving user files', async () => {
      ...
      const migratedSkill = await fs.readFile(
        path.join(testDir, '.kimi-code', 'skills', 'warpweave-explore', 'SKILL.md'),
        'utf-8'
      );
      expect(migratedSkill).toContain('name: warpweave-explore');
      ...
      // its documented /skill:<name> invocations, never /ww:* commands
      // that were not generated
      expect(migratedSkill).not.toContain('/ww:');
      expect(migratedSkill).not.toContain('/ww-');
      expect(migratedSkill).toContain('/skill:warpweave-');
    ...
    it('should remove the legacy .kimi directory entirely when it only held Warpweave skills', async () => {
      const legacySkillDir = path.join(testDir, '.kimi', 'skills', 'warpweave-explore');
      await fs.mkdir(legacySkillDir, { recursive: true });
      await fs.writeFile(
        path.join(legacySkillDir, 'SKILL.md'),
      ...
      const migratedSkill = path.join(testDir, '.kimi-code', 'skills', 'warpweave-explore', 'SKILL.md');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'warpweave-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'warpweave-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('command updates', () => {
    it('heals stale colon references for a filename-invoked tool (cursor)', async () => {
      ...
      // fix carries /ww: references that Cursor's palette never registers.
      // `warpweave update` must rewrite both the command bodies and the skills.
      const initCommand = new InitCommand({ tools: 'cursor', force: true });
      await initCommand.execute(testDir);

      const commandFile = path.join(testDir, '.cursor', 'commands', 'ww-apply.md');
      ...
      for (const file of [commandFile, skillFile]) {
        const stale = (await fs.readFile(file, 'utf-8')).replace(/\/ww-/g, '/ww:');
        await fs.writeFile(file, stale);
      }
      expect(await fs.readFile(commandFile, 'utf-8')).toContain('/ww:apply');
      expect(await fs.readFile(skillFile, 'utf-8')).toContain('/ww:apply');

      await new UpdateCommand({ force: true }).execute(testDir);

      const command = await fs.readFile(commandFile, 'utf-8');
      expect(command).toContain('/ww-archive');
      expect(command).not.toContain('/ww:');
      ...
      expect(skill).toContain('/ww-apply');
      expect(skill).not.toContain('/ww:');
    });
    ...
    it('keeps namespaced references for claude while hyphenating qwen in one run', async () => {
      ...
      const claudeCommand = await fs.readFile(
        path.join(testDir, '.claude', 'commands', 'ww', 'apply.md'),
        'utf-8'
      );
      expect(claudeCommand).toContain('/ww:archive');
      expect(claudeCommand).not.toContain('/ww-archive');

      const qwenCommand = await fs.readFile(
        path.join(testDir, '.qwen', 'commands', 'ww-apply.md'),
        'utf-8'
      );
      expect(qwenCommand).toContain('/ww-archive');
      expect(qwenCommand).not.toContain('/ww:');

      const qwenSkill = await fs.readFile(
        path.join(testDir, '.qwen', 'skills', 'warpweave-apply-change', 'SKILL.md'),
        'utf-8'
      );
      expect(qwenSkill).toContain('/ww-apply');
      expect(qwenSkill).not.toContain('/ww:');

      const claudeSkill = await fs.readFile(
        path.join(testDir, '.claude', 'skills', 'warpweave-apply-change', 'SKILL.md'),
        'utf-8'
      );
      expect(claudeSkill).toContain('/ww:apply');
    ...
    it('heals stale slash references for a prompt-library tool (amazon-q)', async () => {
      ...
      // carries /ww: references that Amazon Q answers to under no spelling.
      ...
      expect(await fs.readFile(promptFile, 'utf-8')).toContain('/ww:apply');
      ...
      for (const file of [promptFile, skillFile]) {
        ...
        expect(refreshed).toContain('@ww-apply');
        expect(refreshed).not.toContain('/ww:');
        expect(refreshed).not.toContain('/ww-');
      }
      // The prompt body cross-references other prompts; those move too.
      expect(await fs.readFile(promptFile, 'utf-8')).toContain('@ww-archive');
    });

    it('should update ww commands for configured Claude tool', async () => {
      ...
      // Check ww command files were created
    ...
    it('should generate ZCode commands under .zcode without creating .agents', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'warpweave-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'warpweave-explore', 'SKILL.md'),
        '---\nmetadata:\n  generatedBy: "0.0.1"\n---\nold content\n'
      );
      ...
      // Commands regenerated under .zcode/commands/ww
    ...
    it('should update core profile ww commands when tool is configured', async () => {
    ...
    it('should refresh both Devin Desktop surfaces with the right invocation syntax', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'warpweave-apply-change'), {
        recursive: true,
      });
      const skillFile = path.join(skillsDir, 'warpweave-apply-change', 'SKILL.md');
      await fs.writeFile(skillFile, 'old content');

      await updateCommand.execute(testDir);

      // Workflows are invoked by filename, so their bodies use `/ww-*`.
      const workflow = path.join(testDir, '.devin', 'workflows', 'ww-apply.md');
      ...
      expect(workflowContent).toContain('/ww-');
      expect(workflowContent).not.toContain('/ww:');
      ...
      expect(skillContent).toContain('/warpweave-apply-change');
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'warpweave-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'warpweave-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'warpweave-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'warpweave-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: warpweave-explore');
      ...
      expect(cursorSkill).toContain('name: warpweave-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'warpweave-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'warpweave-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (Markdown) - Qwen uses flat path structure: ww-<id>.md
    ...
    it('should migrate a legacy .windsurf install to .devin, preserving user files', async () => {
      // A project set up before the Devin Desktop rebrand: Warpweave skills and
      // workflows under .windsurf/, alongside files the user wrote themselves.
      const legacySkillDir = path.join(testDir, '.windsurf', 'skills', 'warpweave-explore');
      await fs.mkdir(legacySkillDir, { recursive: true });
      ...
      await fs.writeFile(path.join(legacyWorkflows, 'ww-explore.md'), 'old workflow content');
      ...
      const migratedSkill = await fs.readFile(
        path.join(testDir, '.devin', 'skills', 'warpweave-explore', 'SKILL.md'),
        'utf-8'
      );
      expect(migratedSkill).not.toContain('old skill content');
      const migratedWorkflow = await fs.readFile(
        path.join(testDir, '.devin', 'workflows', 'ww-explore.md'),
        'utf-8'
      );
      expect(migratedWorkflow).not.toContain('old workflow content');
      expect(migratedWorkflow).toContain('---');

      // The Warpweave-managed originals are gone; the user's files are not
      await expect(fs.access(legacySkillDir)).rejects.toThrow();
      await expect(
        fs.access(path.join(legacyWorkflows, 'ww-explore.md'))
      ).rejects.toThrow();
      expect(await fs.readFile(path.join(userSkillDir, 'SKILL.md'), 'utf-8')).toBe('user skill');
      expect(
    ...
    it.skipIf(process.platform === 'win32', 'should not delete the install when the legacy root is a symlink to the current one', async () => {
      ...
      await updateCommand.execute(testDir);
      const devinSkill = path.join(testDir, '.devin', 'skills', 'warpweave-explore');
    ...
    it('should keep user files that live inside an Warpweave-managed skill directory', async () => {
      ...
      const legacySkill = path.join(testDir, '.windsurf', 'skills', 'warpweave-explore');
    ...
    it('should report divergent files even when nothing is movable', async () => {
      ...
      await fs.writeFile(path.join(devinWorkflows, 'ww-explore.md'), 'current');
      ...
      const legacyWorkflows = path.join(testDir, '.windsurf', 'workflows');
      await fs.mkdir(legacyWorkflows, { recursive: true });
      await fs.writeFile(path.join(legacyWorkflows, 'ww-explore.md'), 'mine');
      ...
      expect(await fs.readFile(path.join(legacyWorkflows, 'ww-explore.md'), 'utf-8')).toBe('mine');
    });
    ...
    it('should keep a legacy SKILL.md the user edited, matching how command files are treated', async () => {
      ...
      const legacyWorkflows = path.join(testDir, '.windsurf', 'workflows');
      await fs.mkdir(legacyWorkflows, { recursive: true });
      await fs.writeFile(path.join(legacyWorkflows, 'ww-explore.md'), 'my edited command');
      ...
      expect(await fs.readFile(path.join(legacyWorkflows, 'ww-explore.md'), 'utf-8')).toBe(
        'my edited command'
      );
    });

    it('should not carry a user file into a skill directory that commands-only delivery deletes', async () => {
      // Only SKILL.md may cross. The destination is a directory Warpweave owns
    ...
    it('should not carry a user file into a skill directory a deselected workflow deletes', async () => {
      // warpweave-new-change is outside the core profile, so the skill
      // directory it would land in is one Warpweave prunes.
      const legacySkill = path.join(testDir, '.windsurf', 'skills', 'warpweave-new-change');
      await fs.mkdir(legacySkill, { recursive: true });
      await fs.writeFile(path.join(legacySkill, 'SKILL.md'), 'stale');
      await fs.writeFile(path.join(legacySkill, 'reference.md'), 'my notes');
    ...
    it('should keep a legacy command file the user edited, and drop an identical one', async () => {
      const devinWorkflows = path.join(testDir, '.devin', 'workflows');
      await fs.mkdir(devinWorkflows, { recursive: true });
      await fs.writeFile(path.join(devinWorkflows, 'ww-explore.md'), 'generated');
      await fs.writeFile(path.join(devinWorkflows, 'ww-apply.md'), 'generated');

      const legacyWorkflows = path.join(testDir, '.windsurf', 'workflows');
      await fs.mkdir(legacyWorkflows, { recursive: true });
      // Edited by the user — deleting it would throw the edit away
      await fs.writeFile(path.join(legacyWorkflows, 'ww-explore.md'), 'my edits');
      // Byte-identical — nothing is lost by dropping it
      await fs.writeFile(path.join(legacyWorkflows, 'ww-apply.md'), 'generated');
      ...
      await expect(fs.access(path.join(legacyWorkflows, 'ww-apply.md'))).rejects.toThrow();
    });
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      await fs.writeFile(
        path.join(skillsDir, 'warpweave-apply-change', 'SKILL.md'),
        'old'
      );
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'warpweave-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'warpweave-explore', 'SKILL.md');
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new Warpweave')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed Warpweave markers from CLAUDE.md')
      );
    ...
    it('should remove managed global Codex ww prompts with --force and preserve unmanaged prompts', async () => {
      setMockConfig({
        featureFlags: {},
        profile: 'core',
        delivery: 'commands',
      ...
      const managedPrompt = path.join(promptDir, 'ww-explore.md');
      ...
      const skillFile = path.join(skillsDir, 'warpweave-explore', 'SKILL.md');
      expect(await FileSystemUtils.fileExists(skillFile)).toBe(true);
      const skillContent = await fs.readFile(skillFile, 'utf-8');
      expect(skillContent).toContain('name: warpweave-explore');
    ...
    it('should infer Codex replacement workflows from legacy prompt filenames during forced update', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.codex', 'skills', 'warpweave-archive-change', 'SKILL.md')
      )).toBe(false);
    });
    ...
    it('should print a skill-based getting-started menu when a legacy upgrade newly configures codex', async () => {
      ...
      // advertise /ww:* commands (codex has no slash surface).
      // The prompt is ww-new.md so the inferred workflow ('new') is one the
      // onboarding menu actually lists — the menu is now filtered to the
      // workflows the upgrade installed.
      const promptDir = path.join(process.env.CODEX_HOME!, 'prompts');
      await fs.mkdir(promptDir, { recursive: true });
      await fs.writeFile(path.join(promptDir, 'ww-new.md'), 'legacy new prompt');
      ...
      expect(menuLines[0]).toContain('$warpweave-new-change');
      expect(logCalls.some((entry) => entry.includes('/ww:new'))).toBe(false);
      expect(logCalls.some((entry) => entry.includes('/ww:continue'))).toBe(false);
      expect(logCalls.some((entry) => entry.includes('/ww:apply'))).toBe(false);
    ...
    it('should print the hyphen getting-started menu when a legacy upgrade newly configures cursor', async () => {
      ...
      // A pre-ww Cursor project: legacy .cursor/commands/openspec-*.md files
      // make the upgrade newly configure cursor, whose menu must name the
      // commands its palette registers (/ww-propose), not /ww:propose.
      ...
      expect(menuLines).toHaveLength(1);
      expect(menuLines[0]).toContain('/ww-propose');
      expect(logCalls.some((entry) => entry.includes('/ww:propose'))).toBe(false);
    });
    ...
    it('should preserve legacy Codex prompts when a configured Codex tool lacks the replacement workflow', async () => {
      ...
      const skillsDir = path.join(testDir, '.codex', 'skills');
      await fs.mkdir(path.join(skillsDir, 'warpweave-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'warpweave-explore', 'SKILL.md'), 'old');

      const promptDir = path.join(process.env.CODEX_HOME!, 'prompts');
      const managedPrompt = path.join(promptDir, 'ww-onboard.md');
      await fs.mkdir(promptDir, { recursive: true });
    ...
    it('should install a missing Codex update skill before removing its prompt in the same forced run', async () => {
      ...
      const promptDir = path.join(process.env.CODEX_HOME!, 'prompts');
      const managedPrompt = path.join(promptDir, 'ww-update.md');
      await fs.mkdir(promptDir, { recursive: true });
      await fs.writeFile(managedPrompt, 'prompt generated by Warpweave v1.6.0');

      const forceUpdateCommand = new UpdateCommand({ force: true });
      await forceUpdateCommand.execute(testDir);

      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'warpweave-update-change', 'SKILL.md')
      )).toBe(true);
      expect(await FileSystemUtils.fileExists(managedPrompt)).toBe(false);
    });
...
`;
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new Warpweave')
      );
    ...
    it('should remove Warpweave marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'warpweave-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + Warpweave markers)
...
`;
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/ww:propose')
      );
      const gettingStartedCalls = consoleSpy.mock.calls
        .map((call) => call.map((arg) => String(arg)).join(' '))
        .join('\n');
      expect(gettingStartedCalls).not.toContain('/ww:new');
      expect(gettingStartedCalls).not.toContain('/ww:continue');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'warpweave-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'warpweave-explore', 'SKILL.md');
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'warpweave-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'warpweave-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should list the expanded commands a custom profile installs', async () => {
      ...
      const output = consoleSpy.mock.calls
        .map((call) => call.map((arg) => String(arg)).join(' '))
        .join('\n');
      expect(output).toContain('/ww:new');
      expect(output).toContain('/ww:continue');
      expect(output).not.toContain('/ww:propose');
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'warpweave-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New ww commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'ww');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'warpweave-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'warpweave-new-change', 'SKILL.md')
      )).toBe(true);
    ...
    it('should list missing core workflows when custom profile preserves the old core workflow set', async () => {
      ...
      expect(calls.some(call =>
        call.includes('Your custom profile is missing 8 core workflows: update, sync, translator, ponytail-minimal-output, superpowers-tdd, security-scan, drift-check, guardrails')
      )).toBe(true);
      expect(calls.some(call =>
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'skills', 'warpweave-sync-specs', 'SKILL.md')
      )).toBe(false);
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'commands', 'ww', 'sync.md')
      )).toBe(false);
    ...
    it('should list missing core workflows when custom profile lacks update and translator', async () => {
      ...
      expect(calls.some(call =>
        call.includes('Your custom profile is missing 7 core workflows: update, translator, ponytail-minimal-output, superpowers-tdd, security-scan, drift-check, guardrails')
      )).toBe(true);
      expect(calls.some(call =>
    ...
    it('should remove skills for configured tools without command adapters in commands-only delivery', async () => {
      ...
      // The tool now has zero Warpweave artifacts; the removal must not be
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'warpweave-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'warpweave-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'ww', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'warpweave-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'warpweave-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'warpweave init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'warpweave init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'warpweave-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'warpweave-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'warpweave-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'warpweave-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'warpweave-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'warpweave-apply-change', 'SKILL.md'), 'content');
