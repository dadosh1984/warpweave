...
describe('resolveWarpweaveRoot', () => {
  ...
  beforeEach(() => {
    tempDir = fs.realpathSync.native(
      fs.mkdtempSync(path.join(os.tmpdir(), 'warpweave-root-selection-'))
    );
  ...
  it('resolves a selected store to its healthy Warpweave root', async () => {
    const storeRoot = await registerStore('team-context');

    const root = await resolveWarpweaveRoot({ store: 'team-context', globalDataDir });
  ...
  it('rejects an unknown store id and lists registered ids', async () => {
    ...
    const error = await expectRootSelectionError(
      resolveWarpweaveRoot({ store: 'team-contxt', globalDataDir }),
      'unknown_store'
    );
  ...
  it('rejects --store when no stores are registered without suggesting --store-path', async () => {
    const error = await expectRootSelectionError(
      resolveWarpweaveRoot({ store: 'team-context', globalDataDir }),
  ...
  it('rejects an invalid store id format before registry lookup', async () => {
    ...
    const error = await expectRootSelectionError(
      resolveWarpweaveRoot({ store: 'Bad/Id', globalDataDir }),
      'invalid_store_id'
    );
  ...
  it('rejects --store-path deliberately with register guidance', async () => {
    const error = await expectRootSelectionError(
      resolveWarpweaveRoot({ storePath: '/somewhere', globalDataDir }),
  ...
  it('resolves the nearest warpweave root without --store', async () => {
    const repoRoot = mkdir('app-repo');
    createOpenSpecRoot(repoRoot);
    const nested = mkdir('app-repo/src/deep');

    const root = await resolveWarpweaveRoot({ startPath: nested, globalDataDir });
  ...
  it('treats workspace state alone as no root at all', async () => {
    ...
    const root = await resolveWarpweaveRoot({ startPath: workspaceDir, globalDataDir });
  ...
  it('fails with a store-selection hint when no root exists but stores are registered', async () => {
    ...
    const error = await expectRootSelectionError(
      resolveWarpweaveRoot({ startPath: appRepo, globalDataDir }),
      'no_root_with_registered_stores'
    );
    expect(error.message).toContain('team-context');
    expect(error.message).toContain('--store <id>');
    expect(error.message).toContain('warpweave init');
  ...
  it('allows an implicit root only when requested', async () => {
    ...
    const implicitRoot = await resolveWarpweaveRoot({ startPath: appRepo, globalDataDir });
    expect(implicitRoot.source).toBe('implicit');
    expect(implicitRoot.path).toBe(appRepo);

    await expectRootSelectionError(
      resolveWarpweaveRoot({ startPath: appRepo, globalDataDir, allowImplicitRoot: false }),
      'no_warpweave_root'
    );
  });
  ...
  describe('declared store fallback (3.2)', () => {
    ...
    it('resolves a config-only pointer to the declared store', async () => {
      ...
      const root = await resolveWarpweaveRoot({ startPath: pointerDir, globalDataDir });
    ...
    it('never overrides a real root and warns once about the ignored pointer', async () => {
      ...
      try {
        const root = await resolveWarpweaveRoot({ startPath: repo, globalDataDir });
        expect(root.source).toBe('nearest');
    ...
    it('keeps config-only directories without a pointer as plain roots', async () => {
      ...
      console.error = (message: string) => warnings.push(String(message));
      try {
        const root = await resolveWarpweaveRoot({ startPath: dir, globalDataDir });
        expect(root.source).toBe('nearest');
    ...
    it('errors on malformed pointers instead of falling through to local writes', async () => {
      ...
      const error = await expectRootSelectionError(
        resolveWarpweaveRoot({ startPath: nonString, globalDataDir }),
      ...
      const yamlError = await expectRootSelectionError(
        resolveWarpweaveRoot({ startPath: unparseable, globalDataDir }),
        'invalid_store_pointer'
      );
      ...
      const scalarRoot = await resolveWarpweaveRoot({ startPath: scalar, globalDataDir });
    ...
    it('treats empty and comments-only configs as plain roots, not malformed pointers', async () => {
      ...
      const emptyRoot = await resolveWarpweaveRoot({ startPath: empty, globalDataDir });
      ...
      const commentedRoot = await resolveWarpweaveRoot({ startPath: commented, globalDataDir });
    ...
    it('prefixes every taxonomy error with the declaration origin, fix unprefixed', async () => {
      ...
      cases.push([
        ...
        () => resolveWarpweaveRoot({ startPath: unknownDir, globalDataDir }),
      ]);
      ...
      cases.push([
        ...
        () => resolveWarpweaveRoot({ startPath: invalidDir, globalDataDir }),
      ]);
      ...
      cases.push([
        ...
        () => resolveWarpweaveRoot({ startPath: unhealthyDir, globalDataDir }),
      ]);
      ...
      cases.push([
        ...
        () => resolveWarpweaveRoot({ startPath: mismatchDir, globalDataDir }),
      ]);
    ...
    it('prefixes no_registered_stores when nothing is registered', async () => {
      ...
      const error = await expectRootSelectionError(
        resolveWarpweaveRoot({ startPath: pointerDir, globalDataDir }),
        'no_registered_stores'
      );
    ...
    it('names a .yml origin when that file was read', async () => {
      ...
      const error = await expectRootSelectionError(
        resolveWarpweaveRoot({ startPath: dir, globalDataDir }),
        'no_registered_stores'
      );
  ...
  describe('global defaultStore fallback (#1359)', () => {
    it('resolves the global defaultStore when no local root or pointer exists', async () => {
      ...
      const root = await resolveWarpweaveRoot({ startPath: scratch, globalDataDir });
    ...
    it('degrades a stale defaultStore to an error that names how to clear it', async () => {
      ...
      expect(error.diagnostic.fix).toContain('warpweave config unset defaultStore');
    });
