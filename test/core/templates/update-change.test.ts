...
describe('update-change templates', () => {
  it('generates the expected skill and command shape (3.1)', () => {
    expect(skill.name).toBe('warpweave-update-change');
    expect(skill.description).toContain('Never edits code');
    expect(skill.license).toBe('MIT');
    expect(skill.compatibility).toBe('Requires warpweave CLI.');
    expect(skill.metadata).toEqual({ author: 'warpweave', version: '1.0' });

    expect(command.name).toBe('WW: Update');
    expect(command.category).toBe('Workflow');
    expect(command.tags).toEqual(['workflow', 'artifacts', 'experimental']);
    expect(command.content).toContain('/ww:update add-auth');

    for (const [label, body] of bodies) {
      expect(body, label).toContain(STORE_SELECTION_GUIDANCE);
      expect(body, label).toContain('warpweave list --json');
      expect(body, label).toContain('warpweave status --change "<name>" --json');
      expect(body, label).toContain('warpweave instructions <artifact-id> --change "<name>" --json');
    }
  });
  ...
  it('edits planning artifacts only, hands code off to /ww:apply, never advances the frontier (3.3)', () => {
    for (const [label, body] of bodies) {
      expect(body, label).toContain('Never edit code');
      expect(body, label).toContain('NEVER edit implementation code');
      expect(body, label).toContain('stop and point to `/ww:apply`');
  ...
  it('ends with next-step guidance and never acts on it (3.5)', () => {
    for (const [label, body] of bodies) {
      ...
      expect(body, label).toContain('suggest `/ww:continue`');
      expect(body, label).toContain('suggest `/ww:apply`');
      expect(body, label).toContain('suggest `/ww:archive`');
      expect(body, label).toContain('the code may no longer match the revised plan');
    }
  });

  it('confirms every edit and redirects intent changes to /ww:new', () => {
    for (const [label, body] of bodies) {
      ...
      expect(body, label).toContain('recommend starting fresh with `/ww:new`');
