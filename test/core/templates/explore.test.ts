...
describe('explore templates', () => {
  ...
  it('loads project context from the Warpweave config at startup (#696)', () => {
    for (const [label, body] of bodies) {
      expect(body, label).toContain('warpweave/config.yaml');
  ...
  it('resolves the config through the reported root rather than assuming a repo-local path (#696)', () => {
    for (const [label, body] of bodies) {
      ...
      expect(body, label).toContain('<root.path>/warpweave/config.yaml');
      expect(body, label).toContain('root.path');
    }
  });

  // resolveConfigFilePath() probes config.yaml then config.yml, and
  // `warpweave init` leaves a .yml project on .yml forever - naming only
