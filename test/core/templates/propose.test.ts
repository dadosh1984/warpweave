...
describe('artifact loop guards (propose and ff)', () => {
  ...
  // agent creates spec files that `warpweave validate` then rejects as
  ...
  it('makes the agent fetch and read the instruction field before skipping', () => {
    for (const [label, body] of loopBodies) {
      expect(body, label).toContain(
        'run `warpweave instructions <artifact-id> --change "<name>" --json` and skip only if its `instruction` field marks it optional'
      );
