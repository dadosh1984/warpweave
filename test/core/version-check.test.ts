...
describe('getAvailableCliUpdate', () => {
  ...
  it('asks the dist-tag endpoint, and never with an Accept type it answers 406 for', async () => {
    ...
    expect(requests[0].url).toBe('/warpweave/latest');
  ...
  it('follows a redirect, as mirrors and corporate front-ends send', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/elsewhere/warpweave/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
  ...
  it('tears down a redirected connection when the overall budget expires', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/mirror/warpweave/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
  ...
  it('asks the registry npm exported, and only that', () => {
    ...
    try {
      ...
      expect(registryUrl()).toBe('https://registry.npmjs.org/warpweave/latest');

      process.env.npm_config_registry = 'https://env.example.com';
      expect(registryUrl()).toBe('https://env.example.com/warpweave/latest');
    } finally {
  ...
  it('falls back to the public registry when the override is not an http(s) URL', () => {
    ...
    expect(registryUrl()).toBe('https://npm.internal.example.com/warpweave/latest');
  });
});
...
describe('offerCliUpgrade', () => {
  ...
  it('recognizes an npm prefix that the node binary does not point at', () => {
    ...
    try {
      ...
      if (isWindows) {
        ...
        fs.writeFileSync(path.join(prefix, 'warpweave.cmd'), '@echo off\n');
      } else {
  ...
  it('reads the version line, not the first version-shaped token in a banner', async () => {
    ...
    try {
      ...
      // report the Node version as Warpweave's.
      fs.writeFileSync(
        bin,
        isWindows
          ? '@echo Node.js v25.8.1 ^| Warpweave\r\n@echo 1.7.0\r\n'
          : '#!/bin/sh\necho "Node.js v25.8.1 | Warpweave"\necho "1.7.0"\n'
      );
...
describe('rerunUpdateWithUpgradedCli', () => {
  ...
  function writeFakeCli(body: string): string {
    const bin = path.join(dir, isWindows ? 'warpweave.cmd' : 'warpweave');
...
describe('displayCliUpdateNote', () => {
  ...
  it('picks the upgrade command that matches how the CLI was installed', () => {
    ...
    expect(globalLines).toContain('npm install -g warpweave@latest');
    ...
    expect(local).toContain('Update the warpweave dependency in this project.');
    ...
    expect(npx).toContain('npx warpweave@latest update');
  ...
  it('does not tell npx users to run an update they were just handed', () => {
    ...
    expect(npx).toEqual(['  npx warpweave@latest update']);
    ...
    expect(buildUpgradeCommandLines(path.join(GLOBAL_ROOT, 'lib', 'node_modules', 'pkg'), PROJECT_ROOT))
      .toContain('  Then run "warpweave update" again to pick up new workflows.');
  });
