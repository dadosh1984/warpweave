...
describe('getAvailableCliUpdate', () => {
  ...
  it('asks the dist-tag endpoint, and never with an Accept type it answers 406 for', async () => {
    ...
    expect(requests[0].url).toBe('/@dadosh1984/spectrix/latest');
  ...
  it('follows a redirect, as mirrors and corporate front-ends send', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/elsewhere/@dadosh1984/spectrix/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
    ...
    await expect(getAvailableCliUpdate()).resolves.toBe(bumpMajor(OPENSPEC_VERSION));
    expect(requests[1].url).toBe('/elsewhere/@dadosh1984/spectrix/latest');
  });
  ...
  it('tears down a redirected connection when the overall budget expires', async () => {
    ...
    respond = (res) => {
      hop += 1;
      if (hop === 1) {
        res.writeHead(302, { location: '/mirror/@dadosh1984/spectrix/latest' });
        res.end();
        return;
      }
      res.writeHead(200, { 'content-type': 'application/json' });
  ...
  it('asks the registry npm exported, and only that', () => {
    ...
    try {
      ...
      expect(registryUrl()).toBe('https://registry.npmjs.org/@dadosh1984/spectrix/latest');

      process.env.npm_config_registry = 'https://env.example.com';
      expect(registryUrl()).toBe('https://env.example.com/@dadosh1984/spectrix/latest');
    } finally {
  ...
  it('falls back to the public registry when the override is not an http(s) URL', () => {
    ...
    expect(registryUrl()).toBe('https://npm.internal.example.com/@dadosh1984/spectrix/latest');
  });
});
...
describe('displayCliUpdateNote', () => {
  ...
  it('picks the upgrade command that matches how the CLI was installed', () => {
    ...
    expect(globalLines).toContain('npm install -g @dadosh1984/spectrix@latest');
    ...
    expect(local).toContain('Update the @dadosh1984/spectrix dependency in this project.');
    ...
    expect(npx).toContain('npx @dadosh1984/spectrix@latest update');
  ...
  it('does not tell npx users to run an update they were just handed', () => {
    ...
    expect(npx).toEqual(['  npx @dadosh1984/spectrix@latest update']);
