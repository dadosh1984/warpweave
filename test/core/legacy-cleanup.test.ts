...
${OPENSPEC_MARKERS.end}`);
  ...
  describe('detectLegacySlashCommands', () => {
    ...
    it('should detect deprecated otrix TOML commands for Qwen', async () => {
      const dirPath = path.join(testDir, '.qwen', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'otrix-explore.toml'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.qwen/commands/otrix-explore.toml');
    });

    it('should not detect new Markdown commands for Qwen as legacy', async () => {
      const dirPath = path.join(testDir, '.qwen', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'otrix-explore.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).not.toContain('.qwen/commands/otrix-explore.md');
    });
    ...
    it('should detect legacy OpenCode otrix-* command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'otrix-propose.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.opencode/command/otrix-propose.md');
    });
    ...
    it('should detect both otrix-* and openspec-* OpenCode command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'otrix-propose.md'), 'content');
      await fs.writeFile(path.join(dirPath, 'openspec-new.md'), 'content');
    ...
    it('should not include managed global Codex prompt files in repo-local slash command detection', async () => {
      ...
      await fs.writeFile(path.join(promptDir, 'otrix-explore.md'), 'legacy explore prompt');
      ...
      expect(result.files).not.toContain(path.join(promptDir, 'otrix-explore.md'));
...
${OPENSPEC_MARKERS.end}`);
  ...
  describe('detectLegacyArtifacts', () => {
    ...
    it('should detect allowlisted global Codex prompts separately from repo-local slash commands', async () => {
      const promptDir = getCodexPromptDir();
      await fs.mkdir(promptDir, { recursive: true });
      await fs.writeFile(path.join(promptDir, 'otrix-explore.md'), 'prompt generated by an older Spectrix version');
      await fs.writeFile(path.join(promptDir, 'otrix-update.md'), 'legacy update prompt');
      await fs.writeFile(path.join(promptDir, 'otrix-review.md'), 'user');
      await fs.writeFile(path.join(promptDir, 'openspec-proposal.md'), 'managed');
      await fs.writeFile(path.join(promptDir, 'my-custom-prompt.md'), 'user');

      const result = await detectLegacyArtifacts(testDir);

      expect(result.globalSlashCommandFiles).toContain(path.join(promptDir, 'otrix-explore.md'));
      expect(result.globalSlashCommandFiles).toContain(path.join(promptDir, 'otrix-update.md'));
      expect(result.globalSlashCommandFiles).not.toContain(path.join(promptDir, 'otrix-review.md'));
      ...
      expect(result.slashCommandFiles).not.toContain(path.join(promptDir, 'otrix-explore.md'));
    });
    ...
    it('should detect exact allowlisted global Codex filenames regardless of template revision', async () => {
      ...
      await fs.mkdir(promptDir, { recursive: true });
      await fs.writeFile(
        path.join(promptDir, 'otrix-explore.md'),
...
${OPENSPEC_MARKERS.end}`);
    ...
    it('should remove allowlisted global Codex prompts and preserve unmanaged prompts', async () => {
      const promptDir = getCodexPromptDir();
      const managedPrompt = path.join(promptDir, 'otrix-apply.md');
      const customOpsxPrompt = path.join(promptDir, 'otrix-review.md');
    ...
    it('should remove exact allowlisted global Codex filenames when their content differs', async () => {
      const promptDir = getCodexPromptDir();
      const customizedManagedName = path.join(promptDir, 'otrix-apply.md');
      await fs.mkdir(promptDir, { recursive: true });
      await fs.writeFile(
    ...
    it('should skip unmanaged global prompt paths in stale detection objects', async () => {
      ...
      const managedPrompt = path.join(promptDir, 'otrix-apply.md');
      const unmanagedPrompt = path.join(promptDir, 'personal.md');
      const outsidePrompt = path.join(testDir, 'other-codex-home', 'prompts', 'otrix-apply.md');
      await fs.mkdir(promptDir, { recursive: true });
  ...
  describe('formatDetectionSummary', () => {
    ...
    it('should format deferred global prompts cleanup separately from repo-local files', () => {
      const globalPrompt = path.join(getCodexPromptDir(), 'otrix-explore.md');
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        ...
        globalSlashCommandDetails: [{
          path: globalPrompt,
          toolId: 'codex',
          managedFileName: 'otrix-explore.md',
          workflowIds: ['explore'],
  ...
  describe('LEGACY_SLASH_COMMAND_PATHS', () => {
    ...
    it('should use the repo-local compatibility glob pattern for Codex prompt detection', () => {
      ...
      expect(patterns).not.toContain('.codex/prompts/otrix-*.md');
    });
  });
  ...
  describe('LEGACY_GLOBAL_SLASH_COMMAND_PATHS', () => {
    it('should define the allowlisted managed global Codex prompt names separately from project-local paths', () => {
      ...
      expect(codexPatterns.managedFileNames).toContain('otrix-explore.md');
      expect(codexPatterns.managedFileNames).toContain('otrix-apply.md');
      expect(codexPatterns.managedFileNames).toContain('otrix-update.md');
      expect(codexPatterns.workflowIdsByFileName?.['otrix-update.md']).toEqual(['update']);
      expect(codexPatterns.managedFileNames).not.toContain('otrix-review.md');
  ...
  describe('getToolsFromLegacyArtifacts', () => {
    ...
    it('should extract codex from managed global legacy prompt files', () => {
      const detection = {
        ...
        slashCommandDirs: [],
        slashCommandFiles: [],
        globalSlashCommandFiles: [path.join(getCodexPromptDir(), 'otrix-explore.md')],
        globalSlashCommandDetails: [{
          path: path.join(getCodexPromptDir(), 'otrix-explore.md'),
          toolId: 'codex',
          managedFileName: 'otrix-explore.md',
          workflowIds: ['explore'],
          replacementLabel: 'Codex skills',
    ...
    it('should handle opencode otrix-* legacy files', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.opencode/command/otrix-propose.md'],
        globalSlashCommandFiles: [],
        hasOpenspecAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
    ...
    it('should deduplicate opencode when both otrix-* and openspec-* files exist', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [
          '.opencode/command/otrix-propose.md',
