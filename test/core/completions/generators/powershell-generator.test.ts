...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for Warpweave CLI');
			expect(script).toContain('$warpweaveCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for warpweave command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize Warpweave',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName warpweave');
			expect(script).toContain('-ScriptBlock $warpweaveCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-WarpweaveChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-WarpweaveSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-WarpweaveSchemas');
			expect(script).toContain('warpweave __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-WarpweaveChanges');
			expect(script).toContain('warpweave __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-WarpweaveSpecs');
			expect(script).toContain('warpweave __complete specs 2>$null');
		});
