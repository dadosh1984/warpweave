...
describe('ZshInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return Oh My Zsh path when Oh My Zsh is installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.oh-my-zsh', 'custom', 'completions', '_warpweave'));
    });
    ...
    it('should return standard Zsh path when Oh My Zsh is not installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.zsh', 'completions', '_warpweave'));
    });
    ...
    it('should honor $ZSH for an Oh My Zsh install at a custom location', async () => {
      ...
      expect(result.isOhMyZsh).toBe(true);
      expect(result.path).toBe(path.join(customRoot, 'custom', 'completions', '_warpweave'));
    });
    ...
    it('should honor $ZSH_CUSTOM over the derived custom dir', async () => {
      ...
      expect(result.path).toBe(
        path.join(testHomeDir, 'dotfiles', 'omz-custom', 'completions', '_warpweave')
      );
    });
  });
  ...
  describe('install', () => {
    const testScript = '#compdef warpweave\n_warpweave() {\n  echo "test"\n}\n';
    ...
    it('should install to Oh My Zsh path when Oh My Zsh is present', async () => {
      ...
      expect(result.isOhMyZsh).toBe(true);
      expect(result.installedPath).toBe(path.join(ohMyZshPath, 'custom', 'completions', '_warpweave'));
    ...
    it('should install to standard Zsh path when Oh My Zsh is not present', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.isOhMyZsh).toBe(false);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.zsh', 'completions', '_warpweave'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.zsh', 'completions', '_warpweave');
    ...
    it('should handle paths with spaces in .zshrc config', async () => {
      ...
      const testHomeDirWithSpaces = await fs.mkdtemp(path.join(os.tmpdir(), 'warpweave zsh test '));
  ...
  describe('uninstall', () => {
    ...
    it('should remove from correct location for Oh My Zsh', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(path.join('.oh-my-zsh', 'custom', 'completions', '_warpweave'));
    });
  });
  ...
  describe('getInstallationInfo', () => {
    ...
    it('should return installation info when installed', async () => {
      ...
      expect(info.path).toContain('_warpweave');
  ...
  describe('configureZshrc', () => {
    ...
    it('should create .zshrc with markers and config when file does not exist', async () => {
      ...
      expect(content).toContain('# WARPWEAVE:START');
      expect(content).toContain('# WARPWEAVE:END');
      expect(content).toContain('# Warpweave shell completions configuration');
    ...
    it('should prepend markers and config when .zshrc exists without markers', async () => {
      ...
      const configIndex = content.indexOf('# WARPWEAVE:START');
  ...
  describe('removeZshrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      expect(newContent).not.toContain('# WARPWEAVE:START');
      expect(newContent).not.toContain('# WARPWEAVE:END');
      expect(newContent).not.toContain('Warpweave shell completions');
    ...
    it('should handle invalid marker placement gracefully', async () => {
      ...
      await fs.writeFile(zshrcPath, '# WARPWEAVE:END\n# WARPWEAVE:START\n');
    ...
    it('should return true when only one marker is present', async () => {
      const zshrcPath = path.join(testHomeDir, '.zshrc');
      await fs.writeFile(zshrcPath, '# WARPWEAVE:START\nsome config\n');
  ...
  describe('install with .zshrc auto-configuration', () => {
    ...
    it('should not configure .zshrc for Oh My Zsh', async () => {
      ...
      expect(content).not.toContain('# WARPWEAVE:START');
  ...
  describe('uninstall with .zshrc cleanup', () => {
    ...
    it('should remove .zshrc config when uninstalling', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain('Removed Warpweave configuration from ~/.zshrc');
