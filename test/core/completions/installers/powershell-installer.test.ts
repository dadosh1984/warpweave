...
describe('PowerShellInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'WarpweaveCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'WarpweaveCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'WarpweaveCompletion.ps1'));
    });
  });
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# WARPWEAVE:END');
      expect(content).not.toContain('# Warpweave completions');
...
Register-ArgumentCompleter -CommandName warpweave -ScriptBlock $openspecCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('WarpweaveCompletion.ps1');
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'warpweave powershell test '));
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('warpweave powershell test');
  ...
  describe('encoding preservation', () => {
    ...
    it('should preserve UTF-16 LE BOM when removing profile config', async () => {
      ...
      const textWithBlock = [
        '. "C:\\Code\\profile.ps1"',
        '# WARPWEAVE:START',
        '. "/path/to/WarpweaveCompletion.ps1"',
        '# WARPWEAVE:END',
        '',
      ].join('\n');
      ...
      // Verify content: original line kept, Warpweave block removed
      const content = raw.subarray(2).toString('utf16le');
    ...
    it('should round-trip UTF-16 LE through install → uninstall without corruption', async () => {
      ...
      // Install adds the Warpweave block
      ...
      // Uninstall removes the Warpweave block
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# WARPWEAVE:START');
      ...
      expect(profileContentAfter).not.toContain('# WARPWEAVE:START');
    });
