...
describe('FishInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return standard fish completions path', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'warpweave.fish'));
    });
    ...
    it('should use homeDir from constructor', () => {
      ...
      expect(result).toBe(path.join(customHome, '.config', 'fish', 'completions', 'warpweave.fish'));
    });
  });
  ...
  describe('install', () => {
    const mockCompletionScript = `# Fish completion script for Warpweave CLI
function __fish_warpweave
    echo "test"
end

complete -c warpweave -a 'init' -d 'Initialize Warpweave'
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'warpweave.fish'));
    ...
    it('should create parent directories if they do not exist', async () => {
      ...
      expect(result.success).toBe(true);
      const targetPath = path.join(testHomeDir, '.config', 'fish', 'completions', 'warpweave.fish');
    ...
    it('should update when content is different', async () => {
      ...
      const updatedScript = `# Fish completion script for Warpweave CLI
function __fish_warpweave_new
    echo "updated"
end

complete -c warpweave -a 'init' -d 'Initialize Warpweave'
complete -c warpweave -a 'validate' -d 'Validate specs'
`;
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'warpweave fish test '));

      const spacedInstaller = new FishInstaller(spacedHomeDir);
      const result = await spacedInstaller.install(mockCompletionScript);

      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('warpweave fish test');
