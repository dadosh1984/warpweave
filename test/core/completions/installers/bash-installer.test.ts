...
describe('BashInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return standard bash-completion path', async () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'warpweave'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '# Bash completion script for Warpweave CLI\n_warpweave_completion() {\n  echo "test"\n}\n';

    it('should install to bash-completion path', async () => {
      const result = await installer.install(testScript);

      expect(result.success).toBe(true);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'warpweave'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'warpweave');
    ...
    it('should handle paths with spaces in .bashrc config', async () => {
      ...
      const testHomeDirWithSpaces = await fs.mkdtemp(path.join(os.tmpdir(), 'warpweave bash test '));
  ...
  describe('removeBashrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      expect(newContent).not.toContain('Warpweave shell completions configuration');
