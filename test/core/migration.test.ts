...
describe('migration', () => {
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'warpweave-explore');
    await writeSkill(projectDir, 'warpweave-apply-change');
  ...
  it('prints the $-prefixed propose reference when migrating a codex-only project', async () => {
    ...
    // /ww:* form
    await writeSkill(projectDir, 'warpweave-propose', '.codex');

    const message = captureMigrationLogs(projectDir, [requireTool('codex')]).find((entry) =>
      entry.includes('New in this version')
    );
    expect(message).toBeTruthy();
    expect(message).toContain('$warpweave-propose');
    expect(message).not.toContain('/warpweave-propose');
    expect(message).not.toContain('/ww:propose');
  });

  it('prints the hyphen propose reference when migrating a qwen-only project', async () => {
    // Qwen invokes commands by filename (.qwen/commands/ww-propose.md ->
    // /ww-propose), so the upgrade message must not advertise the colon form
    ...
    expect(message).toContain('/ww-propose');
    expect(message).not.toContain('/ww:propose');
  });
  ...
  it('prints the @ propose reference when migrating an amazon-q-only project', async () => {
    ...
    // @ww-propose. It registers no slash command, so the upgrade message
  ...
  it('falls back to the skill name when amazon-q and a slash tool disagree', async () => {
    // @ww-propose and /ww-propose are both "flat", so a style-only model
    ...
    expect(message).toContain('the warpweave-propose skill');
    expect(message).not.toContain('@ww-propose');
    expect(message).not.toContain('/ww-propose');
  });

  it('falls back to the skill name when a namespaced and a flat tool disagree', async () => {
    // Claude registers /ww:propose, Qwen registers /ww-propose: no single
  ...
  it('prints the documented /skill: propose reference when migrating a kimi-only project', async () => {
    await writeSkill(projectDir, 'warpweave-propose', '.kimi-code');

    const message = captureMigrationLogs(projectDir, [requireTool('kimi')]).find((entry) =>
      entry.includes('New in this version')
    );
    expect(message).toContain('/skill:warpweave-propose');
    expect(message).not.toContain('/ww:propose');
  });
  ...
  it('falls back to a syntax-neutral reference when command and skill-only tools mix (claude+kimi)', async () => {
    // Claude will get /ww:* commands but Kimi cannot invoke them; the one
  ...
  it('does not advertise /ww:propose when explicit delivery is skills', async () => {
    ...
    await writeSkill(projectDir, 'warpweave-propose');
  ...
  it('advertises /ww:propose when commands are installed for an adapter-backed tool', async () => {
    await writeManagedCommand(projectDir, 'propose');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'ww', 'my-custom.md');
