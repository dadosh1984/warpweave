...
describe('tool-detection', () => {
  ...
  describe('SKILL_NAMES', () => {
    it('should contain all skill names matching COMMAND_IDS', () => {
      expect(SKILL_NAMES).toHaveLength(22);
      ...
      expect(SKILL_NAMES).toContain('warpweave-apply-change');
      expect(SKILL_NAMES).toContain('warpweave-update-change');
      expect(SKILL_NAMES).toContain('warpweave-ff-change');
      expect(SKILL_NAMES).toContain('warpweave-sync-specs');
      expect(SKILL_NAMES).toContain('warpweave-archive-change');
      expect(SKILL_NAMES).toContain('warpweave-bulk-archive-change');
      expect(SKILL_NAMES).toContain('warpweave-verify-change');
      expect(SKILL_NAMES).toContain('warpweave-onboard');
      expect(SKILL_NAMES).toContain('warpweave-propose');
      expect(SKILL_NAMES).toContain('warpweave-ladder-audit');
      expect(SKILL_NAMES).toContain('warpweave-guardrails');
      expect(SKILL_NAMES).toContain('warpweave-debt-ledger');
      expect(SKILL_NAMES).toContain('warpweave-token-budget');
      expect(SKILL_NAMES).toContain('warpweave-benchmark');
      expect(SKILL_NAMES).toContain('warpweave-dependency-check');
      expect(SKILL_NAMES).toContain('warpweave-parallel-execute');
      expect(SKILL_NAMES).toContain('warpweave-learn');
      expect(SKILL_NAMES).toContain('warpweave-init-unified');
  ...
  describe('getToolSkillStatus', () => {
    ...
    it('should detect when one skill exists', async () => {
      const skillDir = path.join(testDir, '.claude', 'skills', 'warpweave-explore');
...
`);
    ...
    // per-tool extension (gemini writes TOML), a flat ww-* file, and — for
    // cline — a directory that is not the tool's skillsDir at all.
    it.each([
      ['gemini', path.join('.gemini', 'commands', 'ww', 'explore.toml')],
      ['cursor', path.join('.cursor', 'commands', 'ww-explore.md')],
      ['cline', path.join('.clinerules', 'workflows', 'ww-explore.md')],
    ...
    it('should detect needsUpdate when a deselected workflow left a command file behind', async () => {
      ...
      const strayFile = path.join(testDir, '.claude', 'commands', 'ww', 'verify.md');
