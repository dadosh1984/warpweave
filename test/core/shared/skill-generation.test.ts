  ...
  describe('generateSkillContent', () => {
    ...
    it('should apply transformInstructions callback when provided', () => {
      const template = {
        ...
        instructions: 'Use /otrix:new to start and /otrix:apply to implement.',
      };

      const transformer = (text: string) => text.replace(/\/otrix:/g, '/otrix-');
      const content = generateSkillContent(template, '0.23.0', transformer);

      expect(content).toContain('/otrix-new');
      expect(content).toContain('/otrix-apply');
      expect(content).not.toContain('/otrix:new');
      expect(content).not.toContain('/otrix:apply');
    });
    ...
    it('should not transform instructions when callback is undefined', () => {
      const template = {
        ...
        instructions: 'Use /otrix:new to start.',
      };
