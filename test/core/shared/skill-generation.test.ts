...
describe('skill-generation', () => {
  describe('getSkillTemplates', () => {
    it('should return all 25 skill templates', () => {
    ...
    it('should include all expected skills', () => {
      ...
      expect(dirNames).toContain('warpweave-explore');
      expect(dirNames).toContain('warpweave-new-change');
      expect(dirNames).toContain('warpweave-continue-change');
      expect(dirNames).toContain('warpweave-apply-change');
      expect(dirNames).toContain('warpweave-update-change');
      expect(dirNames).toContain('warpweave-ff-change');
      expect(dirNames).toContain('warpweave-sync-specs');
      expect(dirNames).toContain('warpweave-archive-change');
      expect(dirNames).toContain('warpweave-bulk-archive-change');
      expect(dirNames).toContain('warpweave-verify-change');
      expect(dirNames).toContain('warpweave-onboard');
      expect(dirNames).toContain('warpweave-propose');
      expect(dirNames).toContain('warpweave-ladder-audit');
      expect(dirNames).toContain('warpweave-guardrails');
      expect(dirNames).toContain('warpweave-debt-ledger');
      expect(dirNames).toContain('warpweave-token-budget');
      expect(dirNames).toContain('warpweave-benchmark');
      expect(dirNames).toContain('warpweave-dependency-check');
    ...
    it('should return single template when filter has one workflow', () => {
      ...
      expect(filtered[0].dirName).toBe('warpweave-propose');
    });
  });

  describe('getCommandTemplates', () => {
    it('should return all 22 command templates', () => {
    ...
    it('should include all expected commands', () => {
      ...
      expect(ids).toContain('security-scan');
    });
  ...
  describe('getCommandContents', () => {
    it('should return all 22 command contents', () => {
  ...
  describe('generateSkillContent', () => {
    ...
    it('should use default values for optional fields', () => {
      ...
      expect(content).toContain('license: MIT');
      expect(content).toContain('compatibility: Requires warpweave CLI.');
      expect(content).toContain('author: warpweave');
    ...
    it('should apply transformInstructions callback when provided', () => {
      const template = {
        ...
        instructions: 'Use /ww:new to start and /ww:apply to implement.',
      };

      const transformer = (text: string) => text.replace(/\/ww:/g, '/ww-');
      const content = generateSkillContent(template, '0.23.0', transformer);

      expect(content).toContain('/ww-new');
      expect(content).toContain('/ww-apply');
      expect(content).not.toContain('/ww:new');
      expect(content).not.toContain('/ww:apply');
    });
    ...
    it('should not transform instructions when callback is undefined', () => {
      const template = {
        ...
        instructions: 'Use /ww:new to start.',
      };
