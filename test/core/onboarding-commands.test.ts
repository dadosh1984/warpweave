...
describe('getOnboardingCommands', () => {
  it('omits commands the profile does not install', () => {
    ...
    expect(commands).toEqual(['/ww:propose', '/ww:apply']);
    expect(commands).not.toContain('/ww:new');
    expect(commands).not.toContain('/ww:continue');
  });
  ...
  it('includes expanded commands when a custom profile installs them', () => {
    ...
    expect(commands).toEqual(['/ww:new', '/ww:continue', '/ww:apply']);
  });
  ...
  it('returns lifecycle order regardless of the order workflows are given', () => {
    ...
    expect(commands).toEqual(['/ww:propose', '/ww:continue', '/ww:apply']);
  });
