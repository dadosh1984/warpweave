...
describe('welcome screen', () => {
  ...
  it('only advertises commands the profile installs', async () => {
    ...
    expect(output).toContain('/ww:propose');
    expect(output).toContain('/ww:apply');
    expect(output).not.toContain('/ww:new');
    expect(output).not.toContain('/ww:continue');
  });
  ...
  it('omits the quick start block when no onboarding workflow is installed', async () => {
    ...
    expect(output).toContain('Welcome to Warpweave');
    expect(output).not.toContain('Quick start after setup:');
  });

  it('does not promise ww commands in the setup summary', async () => {
    const { showWelcomeScreen } = await import('../../src/ui/welcome-screen.js');
    renderStatically();
    ...
    // not state that ww slash commands are part of every setup.
    await showWelcomeScreen(['archive']);
    ...
    expect(output).not.toContain('ww slash commands');
  });
  ...
  it('flags that the quick-start spelling varies by tool', async () => {
    ...
    // prompt before tools are picked — an Amazon Q user types @ww-propose
    // and a Codex user $warpweave-propose, neither of which is shown here.
  ...
  it.runIf(process.platform === 'darwin' || process.platform === 'linux')(
    ...
    async () => {
      ...
      expect(useKeypressMock).toHaveBeenCalledOnce();
      expect(writtenOutput()).toContain('Welcome to Warpweave');
    }
  );
});
