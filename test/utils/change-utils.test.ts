...
describe('createChange', () => {
  ...
  describe('creates directory', () => {
    it('should create change directory', async () => {
      ...
      const changeDir = path.join(testDir, 'warpweave', 'changes', 'add-auth');
    ...
    it('should create a numeric-prefixed change directory (#850, #1169)', async () => {
      ...
      const changeDir = path.join(testDir, 'warpweave', 'changes', '100-add-feature');
      const stats = await fs.stat(changeDir);
      expect(stats.isDirectory()).toBe(true);
    });

    it('should create .warpweave.yaml metadata file with default schema', async () => {
      await createChange(testDir, 'add-auth');

      const metaPath = path.join(testDir, 'warpweave', 'changes', 'add-auth', '.warpweave.yaml');
    ...
    it('should use the process local date across a UTC date boundary', async () => {
      ...
      const metaPath = path.join(testDir, 'warpweave', 'changes', 'local-date-change', '.warpweave.yaml');
      const content = await fs.readFile(metaPath, 'utf-8');
    ...
    it('should preserve the date when UTC and local calendar dates match', async () => {
      ...
      const metaPath = path.join(testDir, 'warpweave', 'changes', 'same-date-change', '.warpweave.yaml');
      const content = await fs.readFile(metaPath, 'utf-8');
      expect(content).toContain('created: 2026-01-05');
    });

    it('should create .warpweave.yaml with custom schema', async () => {
  ...
  describe('creates parent directories if needed', () => {
    it('should create warpweave/changes/ directories if they do not exist', async () => {
      const newProjectDir = path.join(testDir, 'new-project');
      await fs.mkdir(newProjectDir);

      // warpweave/changes/ does not exist yet
      await createChange(newProjectDir, 'add-auth');

      const changeDir = path.join(newProjectDir, 'warpweave', 'changes', 'add-auth');
      const stats = await fs.stat(changeDir);
      expect(stats.isDirectory()).toBe(true);
    });
  });
