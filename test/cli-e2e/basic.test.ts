...
describe('warpweave CLI e2e basics', () => {
  it('shows help output', async () => {
    const result = await runCLI(['--help']);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Usage: warpweave');
    expect(result.stderr).toBe('');
  ...
  describe('init command non-interactive options', () => {
    it('initializes with --tools all option', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('Warpweave Setup Complete');

      // Check that skills were created for multiple tools
      const claudeSkillPath = path.join(emptyProjectDir, '.claude/skills/warpweave-explore/SKILL.md');
      const cursorSkillPath = path.join(emptyProjectDir, '.cursor/skills/warpweave-explore/SKILL.md');
    ...
    it('initializes with --tools agents option', async () => {
      ...
      const skillPath = path.join(emptyProjectDir, '.agents', 'skills', 'warpweave-explore', 'SKILL.md');
  ...
  describe('archive with no terminal to answer its prompts (#1479)', () => {
    ...
    async function prepareChange(options: { tasksComplete?: boolean } = {}): Promise<string> {
      const base = await fs.mkdtemp(path.join(tmpdir(), 'warpweave-archive-e2e-'));
      tempRoots.push(base);
      const changeDir = path.join(base, 'warpweave', 'changes', 'add-greeting');
      await fs.mkdir(path.join(changeDir, 'specs', 'greeting'), { recursive: true });
      await fs.mkdir(path.join(base, 'warpweave', 'specs'), { recursive: true });
      await fs.writeFile(
    ...
    it('reports the flag to pass instead of a closed-prompt error', async () => {
      ...
      expect(output).toContain('warpweave archive add-greeting --yes');

      // The change is untouched: nothing was archived or merged.
      expect(await fileExists(path.join(projectDir, 'warpweave', 'changes', 'add-greeting', 'proposal.md'))).toBe(true);
      expect(await fileExists(path.join(projectDir, 'warpweave', 'specs', 'greeting', 'spec.md'))).toBe(false);
    });
    ...
    it('keeps the caller\'s own flags in the suggested rerun', async () => {
      ...
      const output = `${result.stdout}${result.stderr}`;
      expect(result.exitCode).toBe(1);
      expect(output).toContain('warpweave archive add-greeting --skip-specs --yes');
    });
    ...
    it('reports the skip-validation prompt the same way', async () => {
      ...
      expect(output).toContain('warpweave archive add-greeting --no-validate --yes');
    });
    ...
    it('archives normally once that flag is passed', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(await fileExists(path.join(projectDir, 'warpweave', 'specs', 'greeting', 'spec.md'))).toBe(true);
    });
    ...
    it('keeps --store in the suggested rerun for a store-rooted change', async () => {
      ...
      const base = await fs.mkdtemp(path.join(tmpdir(), 'warpweave-archive-store-e2e-'));
      tempRoots.push(base);
      const env = {
      ...
      const changeDir = path.join(storeRoot, 'warpweave', 'changes', 'add-greeting');
      ...
      const output = `${result.stdout}${result.stderr}`;
      expect(result.exitCode).toBe(1);
      expect(output).toContain('warpweave archive add-greeting --yes --store team-store');
    });
    ...
    it('keeps --store in front of the `--` for a dash-leading change name', async () => {
      ...
      // (behind it).
      const base = await fs.mkdtemp(path.join(tmpdir(), 'warpweave-archive-store-dash-e2e-'));
      tempRoots.push(base);
      const env = {
        XDG_DATA_HOME: path.join(base, 'data'),
        XDG_CONFIG_HOME: path.join(base, 'config'),
      ...
      const changeDir = path.join(storeRoot, 'warpweave', 'changes', '--force');
      await fs.mkdir(changeDir, { recursive: true });
      await fs.writeFile(path.join(changeDir, 'tasks.md'), '- [ ] Task 1\n');
      ...
      const output = `${result.stdout}${result.stderr}`;
      expect(result.exitCode).toBe(1);
      expect(output).toContain('warpweave archive --yes --store team-store -- --force');
    });
  });
});
