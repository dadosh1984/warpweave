...
/**
 * `warpweave view` used to hard-code '.' as its target, so a project whose
 * openspec/config.yaml points at an external store rendered an empty dashboard
 * while `warpweave list` read the store correctly. These cover the fix and the
...
beforeAll(async () => {
  ...
  const specDir = path.join(storeRoot, 'warpweave', 'specs', 'billing');
...
describe('warpweave view root resolution', () => {
  it(
  ...
  it(
    ...
    async () => {
      ...
      expect(view.stdout).toContain('Warpweave Dashboard');
    },
    TIMEOUT_MS
  );
  ...
  it(
    'reports a missing warpweave directory outside any project',
    async () => {
