...
describe('standalone store lifecycle journey', () => {
  it('machine A: setup produces a committed, clonable repo', async () => {
    ...
    expect(log).toContain(`Initialize Warpweave store ${STORE_ID}`);
    ...
    expect(committedFiles).toContain('.warpweave-store/store.yaml');
    expect(committedFiles).toContain('warpweave/specs/.gitkeep');
    expect(committedFiles).toContain('warpweave/changes/archive/.gitkeep');
  ...
  it('machine A: doctor and list see a healthy store with git facts', async () => {
    ...
    expect(store.warpweave_root.healthy).toBe(true);
  ...
  it('machine A: works a change through archive from the project repo', async () => {
    ...
    expect(status.stderr).toContain(`Using Warpweave root: ${STORE_ID}`);
    ...
    expect(instructions.stdout).toContain(
      path.join(canonical(storeRoot), 'warpweave', 'changes', changeId, 'proposal.md')
    );

    // The test acts as the agent and writes the artifacts.
    const changeDir = path.join(storeRoot, 'warpweave', 'changes', changeId);
  ...
  it('machine B: a clone registers without ceremony and reads promoted specs', async () => {
    ...
    expect(doctor.exitCode).toBe(0);
    expect(JSON.parse(doctor.stdout).stores[0].warpweave_root.healthy).toBe(true);
  ...
  it('machine B: completes its own change through archive in the clone', async () => {
    ...
    expect(created.exitCode).toBe(0);
    expect(created.stderr).toContain(`Using Warpweave root: ${STORE_ID}`);
    ...
    expect(instructions.exitCode).toBe(0);
    expect(instructions.stdout).toContain(
      path.join(canonical(cloneRoot), 'warpweave', 'changes', changeId, 'proposal.md')
    );

    const changeDir = path.join(cloneRoot, 'warpweave', 'changes', changeId);
    ...
    const specPath = path.join(cloneRoot, 'warpweave', 'specs', 'invoicing', 'spec.md');
    ...
    expect(failedApply.stderr).toContain(`Using Warpweave root: ${STORE_ID}`);
    expect(failedApply.stderr).toContain(`warpweave new change <name> --store ${STORE_ID}`);
  }, JOURNEY_TIMEOUT_MS);

  it('end state is just normal Warpweave files in both checkouts', async () => {
    for (const root of [storeRoot, cloneRoot]) {
      const entries = await listRelativeEntries(root, new Set(['.git']));

      for (const entry of entries) {
        expect(entry).toMatch(/^(\.warpweave-store(\/|\/store\.yaml)?|warpweave(\/.*)?)$/);
        expect(entry).not.toMatch(/initiative|workspace/i);
      }

      expect(entries).toContain('.warpweave-store/store.yaml');
      expect(entries).toContain('warpweave/config.yaml');
    }
    ...
    for (const env of [machineA, machineB]) {
      const dataEntries = await listRelativeEntries(
        path.join(env.XDG_DATA_HOME as string, 'warpweave'),
        new Set()
      );
