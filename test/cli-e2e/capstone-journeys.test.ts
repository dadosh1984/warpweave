...
describe('capstone persona journeys (6.1)', () => {
  ...
  it('journey 2 — layered flow: app-repo agent discovers, cites, designs locally', async () => {
    ...
    expect(member).toEqual(
      expect.objectContaining({
        ...
        id: 'product-requirements',
        path: storeRoot,
        fetch: 'warpweave show <spec-id> --type spec --store product-requirements',
      })
    );
    ...
    expect(
      fs.existsSync(path.join(storeRoot, 'warpweave', 'changes', 'implement-invoice-immutability'))
    ).toBe(false);

    // The store stayed read-only context throughout.
    const storeChanges = fs.readdirSync(path.join(storeRoot, 'warpweave', 'changes'));
  ...
  it('journey 3 — externalized planning: pointer repo runs the lifecycle without --store', async () => {
    ...
    fs.mkdirSync(path.join(codeRepo, 'warpweave'), { recursive: true });
    fs.writeFileSync(
      path.join(codeRepo, 'warpweave', 'config.yaml'),
      'store: team-planning\n'
    );
    ...
    expect(created.exitCode).toBe(0);
    const changeDir = path.join(storeRoot, 'warpweave', 'changes', 'add-rate-limits');
    expect(fs.existsSync(changeDir)).toBe(true);
    ...
    expect(writtenArtifacts).toEqual(['.warpweave.yaml', 'design.md', 'proposal.md', 'specs', 'tasks.md']);
    ...
    const archiveDir = path.join(storeRoot, 'warpweave', 'changes', 'archive');
    ...
    expect(fs.readdirSync(path.join(codeRepo, 'warpweave'))).toEqual(['config.yaml']);
  }, JOURNEY_TIMEOUT_MS);
});
