...
describe('store git lifecycle', () => {
  ...
  it('defaults to Git without prompting in interactive setup', async () => {
    ...
    const committed = execFileSync('git', ['log', '--format=%s'], { cwd: storeRoot })
      .toString()
      .trim();
    expect(committed).toBe('Initialize Warpweave store interactive-context');
  ...
  it('commits the full store shape when initializing Git on an existing root', async () => {
    ...
    fs.writeFileSync(path.join(storeRoot, 'warpweave', 'specs', 'keep-me.md'), 'user spec\n');
  ...
  it('registers a clone before any changes exist', async () => {
    ...
    expect(fs.existsSync(path.join(cloneRoot, 'warpweave', 'changes'))).toBe(false);
    expect(fs.existsSync(path.join(cloneRoot, 'warpweave', 'specs'))).toBe(false);
  ...
  it('registers a clone with active changes before specs or archive exist', async () => {
    ...
    fs.writeFileSync(
      path.join(storeRoot, 'warpweave', 'changes', 'add-widget', 'tasks.md'),
      '# Tasks\n'
    );
    ...
    expect(committedFiles).toEqual([
      '.warpweave-store/store.yaml',
      'warpweave/changes/add-widget/proposal.md',
      'warpweave/changes/add-widget/tasks.md',
      'warpweave/config.yaml',
    ]);
    expect(committedFiles).not.toContain('warpweave/specs/.gitkeep');
    expect(committedFiles).not.toContain('warpweave/changes/archive/.gitkeep');
    ...
    expect(fs.existsSync(path.join(cloneRoot, 'warpweave', 'specs'))).toBe(false);
    expect(fs.existsSync(path.join(cloneRoot, 'warpweave', 'changes', 'archive'))).toBe(false);
  ...
  it('flags clone-fragile directories and commitless clones', async () => {
    ...
    createHealthyOpenSpecRoot(storeRoot);
    execFileSync('git', ['init'], { cwd: storeRoot, stdio: 'ignore' });
    execFileSync('git', ['add', 'warpweave/config.yaml'], { cwd: storeRoot, env: gitExecEnv });
    ...
    expect(store.status).toEqual([
      expect.objectContaining({
        ...
        message: expect.stringContaining('warpweave/specs/'),
      }),
    ]);
