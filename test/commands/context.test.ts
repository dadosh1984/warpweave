...
describe('warpweave context (4.1)', () => {
  let tempDir: string;
  ...
  it('assembles the working set from declarations, all session shapes', async () => {
    ...
    expect(workingSet.members).toEqual([
      {
        ...
        id: 'upstream-context',
        path: upstream,
        fetch: 'warpweave show <spec-id> --type spec --store upstream-context',
        status: [],
      },
      {
        role: 'referenced_store',
    ...
    expect(human.stdout).toContain('Fetch: warpweave show <spec-id> --type spec --store upstream-context');
