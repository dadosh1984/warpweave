...
async function runStoreCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'warpweave', 'store', ...args]);
}
...
describe('store command', () => {
  ...
  function expectHealthyOpenSpecRoot(root: string): void {
    expect(fs.existsSync(path.join(root, 'warpweave', 'config.yaml')) || fs.existsSync(path.join(root, 'warpweave', 'config.yml'))).toBe(true);
    expect(fs.existsSync(path.join(root, 'warpweave', 'specs'))).toBe(true);
    expect(fs.existsSync(path.join(root, 'warpweave', 'changes'))).toBe(true);
    expect(fs.existsSync(path.join(root, 'warpweave', 'changes', 'archive'))).toBe(true);
  }
  ...
  it('sets up a store at an explicit path without Git in non-interactive JSON mode', async () => {
    ...
    expect(payload.created_files).toEqual([
      ...
      'warpweave/specs/.gitkeep',
      'warpweave/changes/archive/.gitkeep',
      '.warpweave-store/store.yaml',
    ]);
    expect(payload.status).toEqual([]);
    expectHealthyOpenSpecRoot(storeRoot);
    expect(fs.readFileSync(path.join(storeRoot, 'warpweave', 'config.yaml'), 'utf-8')).toContain(
  ...
  it('runs guided setup when no args are passed in an interactive terminal', async () => {
    ...
    expect(input).toHaveBeenCalledWith(expect.objectContaining({
      ...
      default: '~/warpweave/guided-context',
    }));
  ...
  it('preserves an existing healthy Warpweave root during setup', async () => {
    const storeRoot = mkdir('team-context');
    createHealthyOpenSpecRoot(storeRoot, 'config.yml');
    fs.writeFileSync(path.join(storeRoot, 'warpweave', 'specs', 'note.md'), 'keep\n');
    ...
    expect(fs.existsSync(path.join(storeRoot, 'warpweave', 'config.yaml'))).toBe(false);
    expect(fs.readFileSync(path.join(storeRoot, 'warpweave', 'config.yml'), 'utf-8')).toBe(
      `schema: ${DEFAULT_WARPWEAVE_SCHEMA}\n`
    );
    expect(fs.readFileSync(path.join(storeRoot, 'warpweave', 'specs', 'note.md'), 'utf-8')).toBe('keep\n');
  });
  ...
  it('refuses to convert a config-only store pointer repo into a store', async () => {
    ...
    fs.mkdirSync(path.join(pointerRoot, 'warpweave'), { recursive: true });
    fs.writeFileSync(path.join(pointerRoot, 'warpweave', 'config.yaml'), 'store: team-context\n');
    ...
    expect(fs.existsSync(path.join(pointerRoot, 'warpweave', 'specs'))).toBe(false);
    expect(fs.existsSync(path.join(pointerRoot, 'warpweave', 'changes'))).toBe(false);
  ...
  it('rejects explicit setup paths inside an existing Git repo in non-interactive mode', async () => {
    ...
    expect(fs.existsSync(getStoreMetadataPath(storeRoot))).toBe(false);
    expect(fs.existsSync(path.join(storeRoot, 'warpweave'))).toBe(false);
  });
  ...
  it('registers a team store before any changes exist', async () => {
    const storeRoot = mkdir('team-context');
    fs.mkdirSync(path.join(storeRoot, 'warpweave'), { recursive: true });
    fs.writeFileSync(
      path.join(storeRoot, 'warpweave', 'config.yaml'),
      `schema: ${DEFAULT_WARPWEAVE_SCHEMA}\n`
    );
    await writeStoreMetadataState(storeRoot, { version: 1, id: 'team-context' });
    ...
    expect(payload.store.id).toBe('team-context');
    expect(payload.created_files).toEqual([]);
    expect(fs.existsSync(path.join(storeRoot, 'warpweave', 'changes'))).toBe(false);
    expect(fs.existsSync(path.join(storeRoot, 'warpweave', 'specs'))).toBe(false);
    expect(fs.existsSync(path.join(storeRoot, 'warpweave', 'changes', 'archive'))).toBe(false);
  });

  it('registers a store with active changes before specs or archive exist', async () => {
    const storeRoot = mkdir('team-context');
    fs.mkdirSync(path.join(storeRoot, 'warpweave', 'changes', 'add-widget'), { recursive: true });
    fs.writeFileSync(
      path.join(storeRoot, 'warpweave', 'changes', 'add-widget', 'proposal.md'),
      '# Proposal\n'
    );
    fs.writeFileSync(
      path.join(storeRoot, 'warpweave', 'config.yaml'),
  ...
  it('requires confirmation before registering a healthy root without identity', async () => {
    ...
    expect(parseJson(confirmed).created_files).toEqual(['.warpweave-store/store.yaml']);
    await expect(readStoreMetadataState(storeRoot)).resolves.toEqual({
      version: 1,
      id: 'team-context',
    });
  ...
  it('writes nothing when interactive register conversion is declined', async () => {
    ...
    expect(confirm).toHaveBeenCalledWith({
      message: "Turn this Warpweave root into store 'team-context'?",
      default: false,
    });
    expect(fs.existsSync(getStoreMetadataPath(storeRoot))).toBe(false);
  ...
  it('reports repeated setup and register as no-op success', async () => {
    const storeRoot = mkdir('team-context');
    createHealthyOpenSpecRoot(storeRoot);
    fs.writeFileSync(path.join(storeRoot, 'warpweave', 'config.yaml'), 'schema: spec-driven\n# user edit\n');
    ...
    expect(fs.readFileSync(path.join(storeRoot, 'warpweave', 'config.yaml'), 'utf-8')).toBe(
      'schema: spec-driven\n# user edit\n'
    );
    await expect(readStoreRegistryState({ globalDataDir })).resolves.toEqual({
      version: 1,
  ...
  it('rejects registry id and alias path conflicts', async () => {
    ...
    fs.rmSync(path.join(firstRoot, '.warpweave-store'), { recursive: true, force: true });
  ...
  it('doctors registered store path, metadata, and Git presence', async () => {
    ...
    expect(byId['healthy-context'].warpweave_root.healthy).toBe(true);
    ...
    expect(byId['missing-context'].warpweave_root.present).toBeNull();
  ...
  it('reports Warpweave root health separately without repairing it', async () => {
    const storeRoot = mkdir('team-context');
    fs.mkdirSync(path.join(storeRoot, 'warpweave', 'specs'), { recursive: true });
    fs.mkdirSync(path.join(storeRoot, 'warpweave', 'changes'), { recursive: true });
    fs.writeFileSync(path.join(storeRoot, 'warpweave', 'changes', 'archive'), 'not a dir\n');
    fs.writeFileSync(path.join(storeRoot, 'warpweave', 'config.yaml'), `schema: ${DEFAULT_WARPWEAVE_SCHEMA}\n`);
    await writeStoreMetadataState(storeRoot, { version: 1, id: 'team-context' });
    await writeStoreRegistryState(
      {
        version: 1,
    ...
    expect(store.warpweave_root.archive.present).toBe(false);
    expect(store.warpweave_root.status[0]).toEqual(
      expect.objectContaining({
        code: 'warpweave_archive_not_directory',
      })
    );
    expect(fs.readFileSync(path.join(storeRoot, 'warpweave', 'changes', 'archive'), 'utf-8')).toBe('not a dir\n');
  });
  ...
  it('register errors are terminal: one-checkout rule, no circular fix texts', async () => {
    ...
    expect(conflictStatus.fix).toContain('warpweave store unregister team-context');
  ...
  describe('committed format and data dir guards', () => {
    ...
    it('pins the committed store metadata literals and the stores data dir', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(fs.existsSync(path.join(storeRoot, '.warpweave-store', 'store.yaml'))).toBe(true);
    ...
    it('registers a store repo created before the rename', async () => {
      ...
      createHealthyOpenSpecRoot(storeRoot);
      const metadataDir = path.join(storeRoot, '.warpweave-store');
  ...
  describe('store group surface', () => {
    it('hints lifecycle attempts under the store group at --store', async () => {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain("unknown command 'new' for 'warpweave store'");
      expect(result.stderr).toContain(
        'setup, register, unregister, remove, list (ls), doctor'
      );
      expect(result.stderr).toContain('warpweave new change billing-rework --store <id>');
    });
    ...
    it('never suggests an invalid command for partial new invocations', async () => {
      ...
      expect(result.stderr).toContain('warpweave new change <change-id> --store <id>');
      expect(result.stderr).not.toContain('warpweave new my-change');
    });
