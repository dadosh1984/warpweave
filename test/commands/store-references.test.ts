...
describe('store references in instructions (3.1)', () => {
    ...
    expect(payload.references).toEqual([
      {
        ...
        fetch: 'warpweave show <spec-id> --type spec --store team-context',
        status: [],
      },
    ]);
  ...
  it('omits the references field entirely when none are declared', async () => {
    fs.writeFileSync(path.join(appRepo, 'warpweave', 'config.yaml'), 'schema: spec-driven\n');
  ...
  it('completes the PM-to-dev layered flow end to end', async () => {
    ...
    const changeDir = path.join(appRepo, 'warpweave', 'changes', 'billing-rework');
    fs.writeFileSync(
