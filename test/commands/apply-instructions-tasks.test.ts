...
describe('generateApplyInstructions task list', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'warpweave-apply-tasks-'));
  ...
  it('reports the totals warpweave list reports for the same change', async () => {
    writeTasks(
    ...
    const instructions = await generateApplyInstructions(tempDir, 'my-change');
    // `warpweave list` reads progress through getTaskProgressForChange, not the
  ...
  it('counts a text-less checkbox toward progress even though it lists none', async () => {
    // Progress must not disagree with `warpweave list` or archive's gate just
