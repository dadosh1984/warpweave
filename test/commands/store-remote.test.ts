...
describe('store canonical remote (3.3)', () => {
  ...
  describe('setup', () => {
    it('records --remote in store.yaml inside the initial commit', async () => {
      ...
      const committed = git(storeRoot, 'show', 'HEAD:.warpweave-store/store.yaml');
      expect(committed).toContain(`remote: ${TEST_NET_URL}`);
      expect(committed).toBe(
        fs.readFileSync(path.join(storeRoot, '.warpweave-store', 'store.yaml'), 'utf-8')
      );
    ...
    it('refuses --remote when store.yaml already exists, naming the hand-edit', async () => {
      ...
      const before = fs.readFileSync(path.join(storeRoot, '.warpweave-store', 'store.yaml'), 'utf-8');
      ...
      expect(status[0].fix).toContain(path.join('.warpweave-store', 'store.yaml'));
      expect(fs.readFileSync(path.join(storeRoot, '.warpweave-store', 'store.yaml'), 'utf-8')).toBe(
        before
      );
    });
  ...
  describe('register', () => {
    function makeUnregisteredStore(name: string, options: { origin?: string; metadataRemote?: string } = {}): string {
      ...
      fs.mkdirSync(path.join(storeRoot, '.warpweave-store'), { recursive: true });
      fs.writeFileSync(
        path.join(storeRoot, '.warpweave-store', 'store.yaml'),
        `version: 1\nid: ${name}\n` +
  ...
  describe('onboarding end to end', () => {
    it('executes the printed clone fix verbatim and continues to a resolved index', async () => {
      ...
      fs.writeFileSync(path.join(originWorktree, 'warpweave', 'specs', '.gitkeep'), '');
      fs.writeFileSync(path.join(originWorktree, 'warpweave', 'changes', 'archive', '.gitkeep'), '');
      fs.mkdirSync(path.join(originWorktree, '.warpweave-store'), { recursive: true });
      fs.writeFileSync(
        path.join(originWorktree, '.warpweave-store', 'store.yaml'),
        'version: 1\nid: team-context\n'
      );
      ...
      fs.mkdirSync(path.join(appRepo, 'warpweave'), { recursive: true });
      fs.writeFileSync(
        path.join(appRepo, 'warpweave', 'config.yaml'),
        'schema: spec-driven\nreferences:\n' +
          `  - { id: team-context, remote: ${originRemote} }\n`
      );
      fs.mkdirSync(path.join(appRepo, 'warpweave', 'specs'), { recursive: true });
      fs.mkdirSync(path.join(appRepo, 'warpweave', 'changes', 'archive'), { recursive: true });
      ...
      const expectedCheckout = path.join(scratchHome, 'warpweave', 'team-context');
      // The quote style is platform-deliberate: POSIX single quotes,
      // win32 double quotes (cmd/PowerShell treat ' as literal).
      const q = process.platform === 'win32' ? '"' : "'";
      expect(fix).toBe(
        `git clone -- ${originRemote} ${q}${expectedCheckout}${q} && warpweave store register ${q}${expectedCheckout}${q} --id team-context`
      );
  ...
  describe('doctor and resolution', () => {
    it('surfaces both remotes, prefers canonical in human output, no new diagnostics', async () => {
      ...
      fs.writeFileSync(path.join(storeRoot, 'warpweave', 'specs', '.gitkeep'), '');
      fs.writeFileSync(path.join(storeRoot, 'warpweave', 'changes', 'archive', '.gitkeep'), '');
      fs.mkdirSync(path.join(storeRoot, '.warpweave-store'), { recursive: true });
      fs.writeFileSync(
        path.join(storeRoot, '.warpweave-store', 'store.yaml'),
        `version: 1\nid: doc-context\nremote: ${canonical}\n`
