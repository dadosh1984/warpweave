...
describe('warpweave doctor (3.6)', () => {
  ...
  it('reports ok everywhere for a healthy store-backed root, all session shapes', async () => {
    ...
    expect(human.stderr).toContain('Using Warpweave root: team-context');
    ...
    fs.writeFileSync(path.join(pointerRepo, 'warpweave', 'config.yaml'), 'store: team-context\n');
    ...
    fs.mkdirSync(path.join(tempDir, 'config', 'warpweave'), { recursive: true });
    fs.writeFileSync(
      path.join(tempDir, 'config', 'warpweave', 'config.json'),
  ...
  it('prints taxonomy errors in human mode instead of stack traces', async () => {
    ...
    expect(result.stderr).toContain('Error: No Warpweave root found');
