...
async function runConfigCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'warpweave', 'config', ...args]);
}
...
describe('deriveProfileFromWorkflowSelection', () => {
  ...
  it('returns custom when selection is a superset of core workflows', async () => {
    const { deriveProfileFromWorkflowSelection } = await import('../../src/commands/config.js');
    expect(deriveProfileFromWorkflowSelection(['propose', 'explore', 'apply', 'update', 'sync', 'archive', 'translator', 'ponytail-minimal-output', 'superpowers-tdd', 'new'])).toBe('custom');
  });

  it('returns core when selection has exactly core workflows in different order', async () => {
    const { deriveProfileFromWorkflowSelection } = await import('../../src/commands/config.js');
    expect(deriveProfileFromWorkflowSelection(['archive', 'sync', 'update', 'apply', 'explore', 'propose', 'translator', 'ponytail-minimal-output', 'superpowers-tdd'])).toBe('core');
  });
});
...
describe('config profile interactive flow', () => {
  ...
  function setupDriftedProjectArtifacts(projectDir: string): void {
    ...
    const exploreSkillPath = path.join(projectDir, '.claude', 'skills', 'warpweave-explore', 'SKILL.md');
    fs.mkdirSync(path.dirname(exploreSkillPath), { recursive: true });
    fs.writeFileSync(exploreSkillPath, 'name: warpweave-explore\n', 'utf-8');
  }
  ...
  function addExtraVerifyWorkflowArtifacts(projectDir: string): void {
    const verifySkillPath = path.join(projectDir, '.claude', 'skills', 'warpweave-verify-change', 'SKILL.md');
    fs.mkdirSync(path.dirname(verifySkillPath), { recursive: true });
    fs.writeFileSync(verifySkillPath, 'name: warpweave-verify-change\n', 'utf-8');

    const verifyCommandPath = path.join(projectDir, '.claude', 'commands', 'ww', 'verify.md');
  ...
  it('delivery-only action should not invoke workflow checkbox prompt', async () => {
    ...
    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['propose', 'explore', 'apply', 'update', 'sync', 'archive', 'translator', 'ponytail-minimal-output', 'superpowers-tdd'] });
  ...
  it('confirmed project apply should update in process without resolving warpweave from PATH', async () => {
    const { saveGlobalConfig, getGlobalConfig } = await import('../../src/core/global-config.js');
    const { select, confirm } = await getPromptMocks();
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Run `warpweave update` in your other projects to apply.');
  });
  ...
  it('confirmed project apply should report the update failure reason', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith('`warpweave update` failed: permission denied');
  ...
  it('core preset should preserve delivery setting', async () => {
    ...
    expect(config.workflows).toEqual(['propose', 'explore', 'apply', 'update', 'sync', 'archive', 'translator', 'ponytail-minimal-output', 'superpowers-tdd']);
