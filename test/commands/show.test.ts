...
describe('top-level show command', () => {
  ...
  const openspecBin = path.join(projectRoot, 'bin', 'ww.js');
  ...
  it('prints hint and non-zero exit when no args and non-interactive', () => {
    ...
    try {
      ...
      expect(stderr).toContain('warpweave show <item>');
      expect(stderr).toContain('warpweave change show');
      expect(stderr).toContain('warpweave spec show');
    } finally {
  ...
  it('resolves a scaffolded change that has no proposal.md yet', async () => {
    // `warpweave new change <name>` writes only .warpweave.yaml, so `show` must
    // resolve the change the same way `list` and `status` already do.
    await fs.mkdir(path.join(changesDir, 'scaffolded'), { recursive: true });
    await fs.writeFile(path.join(changesDir, 'scaffolded', '.warpweave.yaml'), 'schema: spec-driven\n', 'utf-8');
    ...
    try {
      ...
      expect(stderr).toContain('warpweave status --change scaffolded');
    } finally {
      process.chdir(originalCwd);
    }
  });
