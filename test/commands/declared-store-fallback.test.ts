...
describe('declared store fallback (3.2)', () => {
  ...
  beforeEach(async () => {
    ...
    fs.mkdirSync(path.join(pointerRepo, 'warpweave'), { recursive: true });
    fs.writeFileSync(
      path.join(pointerRepo, 'warpweave', 'config.yaml'),
      'store: team-context\n'
    );
  });
  ...
  it('runs the externalized-planning journey without --store anywhere', async () => {
    ...
    expect(statusHuman.stderr).toContain('Using Warpweave root: team-context');
    ...
    const changeDir = path.join(storeRoot, 'warpweave', 'changes', 'billing-rework');
    fs.writeFileSync(
    ...
    const archived = fs.readdirSync(path.join(storeRoot, 'warpweave', 'changes', 'archive'));
  ...
  it('refuses init in a pointer repo and creates nothing, then converts cleanly', async () => {
    ...
    fs.writeFileSync(path.join(pointerRepo, 'warpweave', 'config.yaml'), 'schema: spec-driven\n');
    const converted = await runCLI(['init', '.', '--tools', 'none'], {
      cwd: pointerRepo,
      env,
    });
    expect(converted.exitCode).toBe(0);
    expect(fs.existsSync(path.join(pointerRepo, 'warpweave', 'specs'))).toBe(true);
    expect(fs.existsSync(path.join(pointerRepo, 'warpweave', 'changes'))).toBe(true);
  });
  ...
  it('refuses init for malformed pointers and from pointer-repo subdirectories', async () => {
    ...
    expect(fs.existsSync(path.join(pointerRepo, 'warpweave', 'specs'))).toBe(false);
    ...
    expect(fs.existsSync(path.join(subdir, 'warpweave'))).toBe(false);
  });
  ...
  it('keeps real-root stdout byte-identical when a pointer is present, with one warning', async () => {
    ...
    for (const [label, config] of [
      ...
      fs.writeFileSync(path.join(realRepo, 'warpweave', 'config.yaml'), config);
