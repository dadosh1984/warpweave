 ...
 * and `warpweave set change` is gone. Existing initiative metadata from the
...
describe('legacy repo-local change initiative metadata', () => {
  ...
  it('keeps reading existing initiative metadata without modifying it', async () => {
    ...
    const metadataPath = path.join(dir, '.warpweave.yaml');
  ...
  it('no longer provides warpweave set change', async () => {
