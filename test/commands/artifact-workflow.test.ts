...
describe('artifact-workflow CLI commands', () => {
  ...
  beforeEach(async () => {
    ...
    changesDir = path.join(tempDir, 'warpweave', 'changes');
  ...
  describe('status command', () => {
    ...
    it('recommends specs before design for a proposal-only change', async () => {
      ...
      expect(json.nextSteps[0]).toContain('warpweave instructions specs');
    });
    ...
    it('exits gracefully when no changes exist', async () => {
      ...
      expect(result.stdout).toContain('warpweave new change');
    });
  ...
  describe('new change command', () => {
    ...
    it('keeps --goal as ordinary metadata without switching schema', async () => {
      ...
      const metadata = await fs.readFile(
        path.join(changesDir, 'goal-change', '.warpweave.yaml'),
        'utf-8'
      );
  ...
  describe('instructions apply command', () => {
    ...
    it('shows blocked state when required artifacts are missing', async () => {
      await fs.writeFile(
        path.join(tempDir, 'warpweave', 'config.yaml'),
        `schema: spec-driven
...
`
    ...
    it('reads a fresh apply config snapshot on every command invocation', async () => {
      const configPath = path.join(tempDir, 'warpweave', 'config.yaml');
...
`
    ...
    it('resolves single-star glob artifacts consistently between status and apply', async () => {
      const schemaDir = path.join(tempDir, 'warpweave', 'schemas', 'glob-test');
...
`
      ...
      await fs.writeFile(path.join(changeDir, '.warpweave.yaml'), 'schema: glob-test\n');
...
`
    ...
    it('fallback: requires all artifacts when schema has no apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'warpweave', 'schemas', 'no-apply');
...
`;
    ...
    it('fallback: ready when all artifacts exist for schema without apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'warpweave', 'schemas', 'no-apply-full');
      const templatesDir = path.join(noApplySchemaDir, 'templates');
      await fs.mkdir(templatesDir, { recursive: true });
...
`
      ...
      expect(
        await fs.readdir(path.join(tempDir, 'warpweave', 'changes'))
      ).toContain('archive-read-only');
      expect(
        await fs
          .stat(path.join(tempDir, 'warpweave', 'specs'))
          .then(() => true)
          .catch(() => false)
      ).toBe(false);
    });
  ...
  describe('experimental command (deprecated alias for init)', () => {
    ...
    it('creates skills for the shared agents target', async () => {
      ...
      const skillFile = path.join(tempDir, '.agents', 'skills', 'warpweave-explore', 'SKILL.md');
    ...
    it('creates skills for Claude tool', async () => {
      ...
      const skillFile = path.join(tempDir, '.claude', 'skills', 'warpweave-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    });
    ...
    it('creates skills for Cursor tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.cursor', 'skills', 'warpweave-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);

      // Verify commands were created with Cursor format
      const commandFile = path.join(tempDir, '.cursor', 'commands', 'ww-explore.md');
    ...
    it('creates skills for the retired windsurf id, under Devin Desktop', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.devin', 'skills', 'warpweave-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    });
  });
  ...
  describe('project config integration', () => {
    describe('new change uses config schema', () => {
      it('creates change with schema from project config', async () => {
        ...
        const metadataPath = path.join(changesDir, 'test-change', '.warpweave.yaml');
      ...
      it('CLI schema overrides config schema', async () => {
        // Create project config with spec-driven schema
        // Note: warpweave directory already exists (from changesDir creation in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'warpweave', 'config.yaml'),
          'schema: spec-driven\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'override-test', '.warpweave.yaml');
        const metadata = await fs.readFile(metadataPath, 'utf-8');
        expect(metadata).toContain('schema: spec-driven');
      }, 60000);
    });
