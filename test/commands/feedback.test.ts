...
describe('FeedbackCommand', () => {
  ...
  describe('gh CLI availability check', () => {
    it('should use which command on Unix/macOS platforms', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/dadosh1984/warpweave/issues/123\n');
  ...
  describe('successful feedback submission', () => {
    it('should submit feedback via gh CLI when authenticated', async () => {
      const issueUrl = 'https://github.com/dadosh1984/warpweave/issues/123';
      ...
      expect(mockExecFileSync).toHaveBeenCalledWith(
        ...
        [
          ...
          'create',
          '--repo',
          'dadosh1984/warpweave',
          '--title',
          'Feedback: Great tool!',
          '--body',
          expect.stringContaining('Submitted via Warpweave CLI'),
          '--label',
          'feedback',
        ],
    ...
    it('should include --body flag when body is provided', async () => {
      const issueUrl = 'https://github.com/dadosh1984/warpweave/issues/124';
    ...
    it('should format title with "Feedback:" prefix', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/dadosh1984/warpweave/issues/125\n');
    ...
    it('should include metadata in issue body', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/dadosh1984/warpweave/issues/126\n');
      ...
      expect(mockExecFileSync).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          '--body',
          expect.stringMatching(/Submitted via Warpweave CLI[\s\S]*Version:[\s\S]*Platform:[\s\S]*Timestamp:/),
        ]),
        expect.any(Object)
      );
    });
    ...
    it('should add feedback label to the issue', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/dadosh1984/warpweave/issues/127\n');
  ...
  describe('error handling', () => {
    ...
    it('should retry without the label when the repo does not define it', async () => {
      const issueUrl = 'https://github.com/dadosh1984/warpweave/issues/129';
    ...
    it('should handle quotes in title and body without escaping (no shell injection)', async () => {
      ...
      mockExecFileSync.mockReturnValue('https://github.com/dadosh1984/warpweave/issues/128\n');
      ...
      const urlCall = consoleLogSpy.mock.calls.find((call: any[]) => {
        ...
        try {
          ...
          return (
            ...
            parsed.pathname === '/dadosh1984/warpweave/issues/new'
          );
        } catch {
          return false;
        }
