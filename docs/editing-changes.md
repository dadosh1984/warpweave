...
If you're using the expanded commands, the natural flow is: edit the artifact, then run `/ww:continue` to pick up from the new state, or `/ww:apply` to keep implementing against the updated plan. If you're on the default `core` commands, edit the artifact and run `/ww:apply`; it reads the current files, so it builds against whatever the artifacts now say.
...
Concretely, after some `/ww:apply` work:
...
- Want a structured check that the code matches the plan? Run `/ww:verify` (expanded command). It reports completeness, correctness, and coherence without blocking anything. See [Workflows: Verify](workflows.md#verify-check-your-work).
...
A fast way to surface mismatches is `/ww:verify`: it reads your artifacts and your code and tells you where they diverge. Treat its output as a to-do list for reconciliation, then archive once they agree.
...
- **Explore first, then re-propose.** If the problem is that the idea itself is unclear, step back to `/ww:explore`, think it through, and let a sharper proposal come out of that. See [Explore First](explore.md).
...
There's a full flowchart and worked examples in [Workflows: When to Update vs Start Fresh](workflows.md#when-to-update-vs-start-fresh) and a deeper treatment in [WW: When to Update vs. Start Fresh](ww.md#when-to-update-vs-start-fresh).

## A note on tasks

`tasks.md` is a living checklist, not a frozen plan. As you implement, you can add tasks you discover, remove ones that turned out unnecessary, or reorder them. The AI checks items off as it completes them during `/ww:apply`, and it resumes from the first unchecked task if you come back later. Editing the list mid-flight is expected.
...
- [Commands](commands.md) - `/ww:continue`, `/ww:apply`, and `/ww:verify` in detail
