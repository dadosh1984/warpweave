...
Because the fix lands as a `MODIFIED` requirement with a fresh scenario, the next person (or the next AI session) sees not just that you fixed it, but what "correct" means. Then `/otrix:apply` and `/otrix:archive` as usual.
...
Start with `/otrix:explore`. It's a thinking partner with no structure and no artifacts created. It reads your codebase and helps you decide.
...
You: /otrix:propose optimize-product-list-fetching
...
Naming the change in `/otrix:apply add-dark-mode` is how you point the AI at a specific change when more than one is active. Because tasks track completion in `tasks.md`, the AI knows exactly where you stopped.

When several changes are done at once, the expanded `/otrix:bulk-archive` files them together and resolves spec conflicts by checking what's actually implemented. See [Workflows](workflows.md#parallel-changes).
...
```text
You: /otrix:propose refactor-payment-module
...
Review each artifact as it lands, edit freely, and continue when you're happy. When you want the rest drafted in one go, `/otrix:ff` fast-forwards through the remaining planning artifacts. Before archiving, `/otrix:verify` checks that the implementation actually matches the specs. See [Workflows](workflows.md#otrixff-vs-otrixcontinue).
...
`/otrix:onboard` finds a real (small) improvement, creates a change for it, implements it, and archives it, narrating every step. It takes 15 to 30 minutes and leaves you with a real change you can keep or discard. It's the gentlest way to learn. See [Commands](commands.md#otrixonboard).
