...
The short answer: a change is just files, and Warpweave never touches git. So it fits your existing workflow instead of replacing it. This page spells out the conventions that work well.

## One rule: Warpweave doesn't touch git

Warpweave reads and writes plain Markdown under `warpweave/`. It never commits, branches, pushes, or pulls in your project — and it never clones or syncs a [store](stores-beta/user-guide.md) on its own. That means:

- **You commit `warpweave/` like any source.** Specs, active changes, and the archive are part of your project's history. (Yes, commit the whole folder — see the [FAQ](faq.md#should-i-commit-the-warpweave-folder-to-git).)
- **A change is a folder you version like code.** `warpweave/changes/add-dark-mode/` is just files on a branch.
- **Everything below is convention, not enforcement.** Warpweave won't make you do it this way; it just fits cleanly.
...
git switch -c add-dark-mode        start a branch, as usual
   │
/otrix:propose add-dark-mode        draft the plan (proposal + specs + tasks)
   │
REVIEW THE PLAN                    you read it before any code — see Reviewing a Change
   │
/otrix:apply                        build it; artifacts + code change together
   │
git commit && open a PR            the PR contains the spec delta AND the code
   │
teammate reviews, merges
   │
/otrix:archive                      fold the delta into specs/, move the change to archive/
```
...
Archiving folds a change's deltas into your main `warpweave/specs/` and moves the change folder to `warpweave/changes/archive/YYYY-MM-DD-<name>/`. Because `specs/` is the **shared source of truth**, the timing matters on a team. Two workable conventions:
...
Pick one and be consistent. Either way, `/otrix:archive` checks that tasks are complete and offers to sync first, so nothing merges half-finished by accident.
...
- **The one place conflicts show up is `specs/`.** If two changes both modify the *same* requirement, archiving the second one will conflict in `warpweave/specs/…/spec.md` — resolve it like any merge conflict, keeping the requirement that reflects reality. This is rare, and it's a feature: it's git telling you two changes disagreed about how the system should behave.

## When planning outgrows one repo

Everything above assumes the plan lives in the code repo's own `warpweave/` folder, which is the right default. When your planning genuinely spans several repos or teams — one feature touching three services, or requirements one team owns and others consume — that's what the beta **stores** feature is for: planning gets its own repo that any code repo can point at. Start with the [Stores User Guide](stores-beta/user-guide.md).
