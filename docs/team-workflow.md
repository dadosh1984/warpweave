...
git switch -c add-dark-mode        start a branch, as usual
   │
/ww:propose add-dark-mode        draft the plan (proposal + specs + tasks)
   │
REVIEW THE PLAN                    you read it before any code — see Reviewing a Change
   │
/ww:apply                        build it; artifacts + code change together
   │
git commit && open a PR            the PR contains the spec delta AND the code
   │
teammate reviews, merges
   │
/ww:archive                      fold the delta into specs/, move the change to archive/
```
...
Pick one and be consistent. Either way, `/ww:archive` checks that tasks are complete and offers to sync first, so nothing merges half-finished by accident.
