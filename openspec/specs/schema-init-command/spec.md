...
The CLI SHALL provide an `warpweave schema init <name>` command that creates a new schema directory under `openspec/schemas/<name>/` with a valid `schema.yaml` file and default template files.
...
- **WHEN** user runs `warpweave schema init existing-schema` and `openspec/schemas/existing-schema/` already exists
