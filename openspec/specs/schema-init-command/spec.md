...
## Purpose
Define `spectrix schema init` behavior for creating project-local schema skeletons in interactive and non-interactive modes.
## Requirements
### Requirement: Schema init command creates project-local schema
The CLI SHALL provide an `spectrix schema init <name>` command that creates a new schema directory under `openspec/schemas/<name>/` with a valid `schema.yaml` file and default template files.

#### Scenario: Create schema with valid name
- **WHEN** user runs `spectrix schema init my-workflow`
...
- **WHEN** user runs `spectrix schema init "My Workflow"` (contains space)
...
- **WHEN** user runs `spectrix schema init existing-schema` and `openspec/schemas/existing-schema/` already exists
...
- **WHEN** user runs `spectrix schema init my-workflow` in an interactive terminal
...
- **WHEN** user runs `spectrix schema init my-workflow --description "My workflow" --artifacts proposal,tasks`
...
- **WHEN** user runs `spectrix schema init my-workflow` in interactive mode
...
- **WHEN** user runs `spectrix schema init my-workflow --json --description "Test" --artifacts proposal`
...
- **WHEN** user runs `spectrix schema init "invalid name" --json`
