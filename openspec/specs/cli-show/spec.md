...
## Purpose
Define top-level `spectrix show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `spectrix show` without arguments
...
- **WHEN** executing `spectrix show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `spectrix show <item>` or `spectrix change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `spectrix show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `spectrix change show`/`spectrix spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `spectrix show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `spectrix show --type spec <item>`
...
- **WHEN** executing `spectrix show <item> --json`
...
- **WHEN** showing a change with `spectrix show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `spectrix show <spec-id> --requirements`
