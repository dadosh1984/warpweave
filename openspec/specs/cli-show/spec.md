...
## Purpose
Define top-level `warpweave show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `warpweave show` without arguments
...
- **WHEN** executing `warpweave show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `warpweave show <item>` or `warpweave change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `warpweave show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `warpweave change show`/`warpweave spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `warpweave show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `warpweave show --type spec <item>`
...
- **WHEN** executing `warpweave show <item> --json`
...
- **WHEN** showing a change with `warpweave show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `warpweave show <spec-id> --requirements`
