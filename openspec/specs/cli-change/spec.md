...
## Purpose
Define `spectrix change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `spectrix change show update-error --json`
...
- **WHEN** executing `spectrix change list`
...
- **WHEN** executing `spectrix change show update-error --requirements-only`
...
- **WHEN** executing `spectrix change validate update-error`
...
- **WHEN** executing `spectrix list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'spectrix list' is deprecated. Use 'spectrix change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `spectrix list --all`
...
- **WHEN** executing `spectrix change show` without arguments
...
- **WHEN** executing `spectrix change show` without a change name
...
- **WHEN** executing `spectrix change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `spectrix change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
