...
As a developer using Spectrix, I want to update the Spectrix instructions in my project when new versions are released, so that I can benefit from improvements to AI agent instructions.
## Requirements
### Requirement: Update Behavior
The update command SHALL update Spectrix instruction files to the latest templates in a team-friendly manner.
...
The command SHALL require an existing Spectrix structure before allowing updates.

#### Scenario: Checking prerequisites

- **GIVEN** the command requires an existing `openspec` directory (created by `spectrix init`)
- **WHEN** the `openspec` directory does not exist
- **THEN** display error: "No Spectrix directory found. Run 'spectrix init' first."
...
The update command SHALL refresh Spectrix-managed files in a predictable manner while respecting each team's chosen tooling.
...
- **AND** update only the Spectrix-managed sections inside existing AI tool files, leaving user-authored content untouched
- **AND** avoid creating new native-tool configuration files (slash commands, CLAUDE.md, etc.) unless they already exist

### Requirement: Core Files Always Updated
The update command SHALL always update the core Spectrix files and display an ASCII-safe success message.
...
- **WHEN** `.opencode/commands/` contains Spectrix-managed `opsx-*.md` command files for the configured profile (for example `opsx-propose.md`, `opsx-apply.md`, and `opsx-archive.md`)
- **THEN** refresh each file using shared templates
...
- **THEN** the template SHALL instruct the AI to validate the provided change ID against `spectrix list`
...
- **THEN** the template SHALL instruct the AI to identify the change ID from context or by running `spectrix list`
