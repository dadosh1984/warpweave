...
## Purpose
Provide shell completion scripts for the Spectrix CLI, enabling tab-completion for commands, flags, and dynamic values (change IDs, spec IDs) across multiple shells. Supports Zsh, Bash, Fish, and PowerShell.
## Requirements
...
- **WHEN** user executes `spectrix completion --help`
...
- **WHEN** user executes `spectrix completion generate zsh`
...
- **WHEN** user executes `spectrix completion generate bash`
...
- **AND** support dynamic completion for change and spec IDs via `spectrix __complete`

#### Scenario: Generating Fish completion

- **WHEN** user executes `spectrix completion generate fish`
...
- **WHEN** user executes `spectrix completion generate powershell`
...
- **AND** only provide suggestions when inside an Spectrix-enabled project
...
- **WHEN** user requests completions outside an Spectrix project
...
- **WHEN** user executes `spectrix completion install zsh`
...
- **AND** write completion script to `~/.oh-my-zsh/custom/completions/_spectrix`
...
- **WHEN** user executes `spectrix completion install zsh` and Oh My Zsh is not detected
- **THEN** create completions directory at `~/.zsh/completions/` if it doesn't exist
- **AND** write completion script to `~/.zsh/completions/_spectrix`
...
- **WHEN** user executes `spectrix completion install bash`
...
- **WHEN** user executes `spectrix completion install fish`
...
- **WHEN** user executes `spectrix completion install powershell`
...
- **WHEN** user executes `spectrix completion install` without specifying a shell
...
- **WHEN** user executes `spectrix completion uninstall zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_spectrix` if Oh My Zsh is detected
- **AND** remove `~/.zsh/completions/_spectrix` if standard Zsh setup is detected
- **AND** remove fpath modifications from `~/.zshrc` using marker-based removal
- **AND** display success message

#### Scenario: Uninstalling Bash completion

- **WHEN** user executes `spectrix completion uninstall bash`
...
- **WHEN** user executes `spectrix completion uninstall fish`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `spectrix completion uninstall powershell`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `spectrix completion uninstall` without specifying a shell
...
- **AND** implement methods:
  ...
  - `isOpenSpecProject(): boolean` - Checks if current directory is Spectrix-enabled
...
- **WHEN** `spectrix completion install` cannot detect current shell
- **THEN** display error: "Could not auto-detect shell. Please specify shell explicitly."
- **AND** display usage hint: "Usage: spectrix completion <operation> [shell]"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `spectrix completion generate zsh > /path/to/_spectrix`
