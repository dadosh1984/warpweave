...
## Purpose
Define `spectrix feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide an `spectrix feedback` command that creates a GitHub Issue in the spectrix repository using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.

#### Scenario: Simple feedback submission

- **WHEN** user executes `spectrix feedback "Great tool!"`
- **THEN** the system executes `gh issue create` with title "Feedback: Great tool!"
- **AND** the issue is created in the spectrix repository
...
- **AND** the issue is created in the spectrix repository without the `feedback` label
- **AND** the system displays the created issue URL
...
- **WHEN** user executes `spectrix feedback "Title here" --body "Detailed description..."`
- **THEN** the system creates a GitHub Issue with the specified title
- **AND** the issue body contains the detailed description
- **AND** the issue body includes metadata (Spectrix version, platform, timestamp)
...
- **WHEN** user runs `spectrix feedback "message"`
...
- **THEN** the issue body includes:
  - Spectrix CLI version
  - Platform (darwin, linux, win32)
  - Submission timestamp
  - Separator line: "---\nSubmitted via Spectrix CLI"
...
- **AND** user runs `spectrix feedback "message"`
...
- **AND** submits via `spectrix feedback` command on user confirmation
...
- **WHEN** user types `spectrix fee<TAB>`
- **THEN** the shell completes to `spectrix feedback`

#### Scenario: Flag completion

- **WHEN** user types `spectrix feedback "msg" --<TAB>`
