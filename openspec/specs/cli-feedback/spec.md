...
## Purpose
Define `warpweave feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide an `warpweave feedback` command that creates a GitHub Issue in the warpweave repository using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.

#### Scenario: Simple feedback submission

- **WHEN** user executes `warpweave feedback "Great tool!"`
- **THEN** the system executes `gh issue create` with title "Feedback: Great tool!"
- **AND** the issue is created in the warpweave repository
...
- **AND** the issue is created in the warpweave repository without the `feedback` label
- **AND** the system displays the created issue URL
...
- **WHEN** user executes `warpweave feedback "Title here" --body "Detailed description..."`
- **THEN** the system creates a GitHub Issue with the specified title
- **AND** the issue body contains the detailed description
- **AND** the issue body includes metadata (Warpweave version, platform, timestamp)
...
- **THEN** the issue body includes:
  - Warpweave CLI version
  - Platform (darwin, linux, win32)
  - Submission timestamp
  - Separator line: "---\nSubmitted via Warpweave CLI"
...
- **AND** user runs `warpweave feedback "message"`
...
- **AND** submits via `warpweave feedback` command on user confirmation
...
- **WHEN** user types `warpweave fee<TAB>`
- **THEN** the shell completes to `warpweave feedback`

#### Scenario: Flag completion

- **WHEN** user types `warpweave feedback "msg" --<TAB>`
