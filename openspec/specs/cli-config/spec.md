...
## Purpose
Provide a user-friendly CLI interface for viewing and modifying global Spectrix configuration settings without manually editing JSON files.
## Requirements
...
- **WHEN** user executes `spectrix config --help`
...
- **WHEN** user executes `spectrix config path`
...
- **WHEN** user executes `spectrix config get <key>` with a valid top-level key
- **THEN** print the raw value only (no labels or formatting)
- **AND** exit with code 0

#### Scenario: Get nested key with dot notation

- **WHEN** user executes `spectrix config get featureFlags.someFlag`
- **THEN** traverse the nested structure using dot notation
- **AND** print the value at that path

#### Scenario: Get non-existent key

- **WHEN** user executes `spectrix config get <key>` with a key that does not exist
- **THEN** print nothing (empty output)
- **AND** exit with code 1

#### Scenario: Get object value

- **WHEN** user executes `spectrix config get <key>` where the value is an object
...
- **WHEN** user executes `spectrix config set <key> <value>`
...
- **WHEN** user executes `spectrix config set <key> true` or `spectrix config set <key> false`
...
- **WHEN** user executes `spectrix config set <key> <value> --string`
...
- **WHEN** user executes `spectrix config set featureFlags.newFlag true`
...
- **WHEN** user executes `spectrix config unset <key>`
...
- **WHEN** user executes `spectrix config reset --all`
...
- **WHEN** user executes `spectrix config reset --all -y`
- **THEN** reset without prompting for confirmation

#### Scenario: Reset without --all flag

- **WHEN** user executes `spectrix config reset` without `--all`
...
- **WHEN** user executes `spectrix config edit`
...
The `spectrix config profile` command SHALL provide an action-first interactive flow that allows users to modify delivery and workflow settings independently.

#### Scenario: Current profile summary appears first

- **WHEN** user runs `spectrix config profile` in an interactive terminal
...
- **WHEN** user runs `spectrix config profile` interactively
...
- **WHEN** delivery selection is shown in `spectrix config profile`
...
- **WHEN** `spectrix config profile` exits with `No config changes.` inside an Spectrix project
...
- **AND** include guidance to run `spectrix update` to sync project files

#### Scenario: Apply prompt is gated on actual changes

- **WHEN** config values were changed and saved
- **AND** current directory is an Spectrix project
- **THEN** prompt `Apply changes to this project now?`
- **AND** if confirmed, run `spectrix update` for the current project
...
- **WHEN** user executes `spectrix config set someFutureKey 123`
...
- **WHEN** user executes `spectrix config set someFutureKey 123 --allow-unknown`
- **THEN** the value is saved successfully
- **AND** exit with code 0

#### Scenario: Invalid feature flag value rejected

- **WHEN** user executes `spectrix config set featureFlags.someFlag notABoolean`
...
- **WHEN** user executes `spectrix config --scope project <subcommand>`
