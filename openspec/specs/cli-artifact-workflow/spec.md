...
- **WHEN** user runs `spectrix status --change <id>`
...
- **WHEN** user runs `spectrix status --change <id> --json`
...
- **WHEN** user runs `spectrix status --change <id>` (text or `--json`)
...
- **WHEN** user runs `spectrix status --change <id>` on a change with no artifacts
...
- **WHEN** user runs `spectrix status` without `--change`
...
- **WHEN** user runs `spectrix status --change unknown-id`
...
The workflow SHALL use `spectrix status` output to determine what can be created next, rather than a separate next-command surface.

#### Scenario: Discover next artifacts from status output

- **WHEN** a user needs to know which artifact to create next
- **THEN** `spectrix status --change <id>` identifies ready artifacts with `[ ]`
...
- **WHEN** user runs `spectrix instructions <artifact> --change <id>`
...
- **WHEN** user runs `spectrix instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `spectrix instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `spectrix instructions proposal --change <id>` on a scaffolded change
...
- **WHEN** user runs `spectrix templates`
...
- **WHEN** user runs `spectrix new change add-feature`
- **THEN** the system creates `openspec/changes/add-feature/` directory

#### Scenario: Invalid change name
- **WHEN** user runs `spectrix new change "Add Feature"` with invalid name
- **THEN** the system displays validation error with guidance

#### Scenario: Duplicate change name
- **WHEN** user runs `spectrix new change existing-change` for an existing change
- **THEN** the system displays an error indicating the change already exists

#### Scenario: Create with description
- **WHEN** user runs `spectrix new change add-feature --description "Add new feature"`
...
- **WHEN** user runs `spectrix status --change <id> --schema tdd`
...
The system SHALL generate schema-aware apply instructions via `spectrix instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `spectrix instructions apply --change <id>`
...
- **WHEN** user runs `spectrix instructions apply --change <id> --json`
- **THEN** the system outputs JSON with:
...
- **WHEN** user runs `spectrix artifact-experimental-setup --tool cursor`
...
- **WHEN** user runs `spectrix artifact-experimental-setup` without `--tool`
...
- **WHEN** user runs `spectrix artifact-experimental-setup --tool unknown-tool`
...
- **WHEN** a user runs `spectrix status --change <id> --json`
...
- **GIVEN** the change is repo-local
- **WHEN** a user runs `spectrix instructions <artifact> --change <id> --json`
- **THEN** instruction output SHALL preserve existing repo-local paths

### Requirement: Workflow skills use CLI artifact context
Generated workflow skills SHALL use Spectrix CLI output as the source of truth for artifact locations.
...
- **THEN** it SHALL instruct the agent to run `spectrix status --change <id> --json`
...
- **THEN** it SHALL instruct the agent to run `spectrix instructions <artifact> --change <id> --json`
