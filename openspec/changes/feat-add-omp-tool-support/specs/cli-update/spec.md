...
- **WHEN** `openspec update` runs and Oh My Pi is a configured tool
- **THEN** command files are written to `.omp/commands/otrix-<id>.md` for each workflow in the active profile, creating them if they do not yet exist and overwriting them if they do
