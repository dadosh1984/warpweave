...
OpenSpec SHALL generate command files for Oh My Pi in `.omp/commands/otrix-<id>.md`, one per active workflow command.

Each file SHALL include a YAML frontmatter block with a `description` field. The command body SHALL transform `/otrix:` references to `/otrix-` to match Oh My Pi's filename-based slash command naming (e.g., `otrix-propose.md` → `/otrix-propose`). It SHALL inject `**Provided arguments**: $@` on the line immediately following any `**Input**:` heading, unless `$@` or `$ARGUMENTS` is already present in the body.
...
- **THEN** the file is written to `.omp/commands/otrix-propose.md`
...
- **WHEN** OpenSpec writes a command file for Oh My Pi whose body contains `/otrix:apply` or similar colon-style references
- **THEN** those references are transformed to `/otrix-apply` in the output file
...
- **AND** when the user invokes `/otrix-propose my-feature`, the agent receives `my-feature` as the value of `$@`
...
Skill file bodies SHALL have `/otrix:` references transformed to `/otrix-` so that skill invocations refer to the correct hyphen-based slash command names.
...
- **WHEN** OpenSpec writes a skill file for Oh My Pi whose body contains `/otrix:explore`
- **THEN** the reference is transformed to `/otrix-explore` in the output file
