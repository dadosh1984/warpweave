# Tasks: `/otrix:update` — a thin update skill
...
- [x] 2.1 Add a `/otrix:update` row to the command table in `docs/otrix.md`, plus a short "Updating a change" usage note.
...
- [x] 3.3 Assert the template instructs planning-artifacts-only with a hand-off to `/otrix:apply` for code, and never advances the build frontier.
...
- [x] 3.5 Assert the template ends with next-step guidance (`/otrix:continue`/`/otrix:apply`/`/otrix:archive`) and instructs the agent never to act on it.
...
- [x] 4.2 Manual walk-through: on a `spec-driven` change, edit `design`, run `/otrix:update`, confirm it proposes coherence edits to other existing artifacts (including upstream where warranted) and never touches code.
