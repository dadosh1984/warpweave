...
OTRIX names **four** first-class actions — "create, implement, **update**, archive — do any of them anytime" ([docs/otrix.md:52](../../../docs/otrix.md)). Three ship as commands. **`update` does not exist.** The only mechanism offered is *"edit the files manually"* — and when you edit one artifact, nothing helps you keep the rest of the change coherent. Worse, the manual workaround lets the agent edit **code** when the user only wanted to revise the **plan** ([#1188](https://github.com/Fission-AI/OpenSpec/issues/1188)).

This is the most-requested missing capability in the tracker. It is one gap with several faces, and the fix is small: a thin `/otrix:update` skill that revises a change's planning artifacts and keeps them coherent with each other, built on the **existing** `openspec status` / `openspec list` commands. No new graph engine, no digests, no ledger — just an agent that reads the change's artifacts and updates what needs updating, with the user's confirmation.

## What Changes

The whole feature is a single new workflow skill, `/otrix:update`. The skill is deliberately change-scoped — `openspec-update-change`, following the `openspec-<verb>-change` naming of its siblings — and applies to change proposals only, not arbitrary artifact graphs (see design, Naming). Written by hand, its instruction set is short:
...
5. **Point to the next step** — report where the change now stands and recommend what comes next: artifacts still missing → `/otrix:continue`; plan revised after the change was already implemented → `/otrix:apply` to carry the delta into code; everything done and implemented → `/otrix:archive`. Guidance only — the skill never acts on it.

Two guardrails make it the command the cluster asked for:

- **Planning artifacts only, never code.** If a revised plan implies code changes, it hands off to `/otrix:apply` ([#1188](https://github.com/Fission-AI/OpenSpec/issues/1188)).
...
- `otrix-update-skill`: A new `/otrix:update` workflow skill that revises a change's existing planning artifacts and keeps them coherent with one another. It reads the artifact set and paths from `openspec status`, reviews related artifacts in any direction (not only downstream), edits planning artifacts only and never code, and confirms each edit with the user. It ends with next-step guidance — recommending `/otrix:continue`, `/otrix:apply`, or `/otrix:archive` based on the change's state — without acting on it.

## Impact

- `src/core/templates/workflows/update-change.ts` (**new**) — the `openspec-update-change` skill template and the `/otrix:update` command template, mirroring the structure of `continue-change.ts`. Reads artifact ids and paths from `openspec status --json`; embeds no artifact-name patterns.
- Skill/command registration + [src/core/profiles.ts](../../../src/core/profiles.ts) — add `update` to `ALL_WORKFLOWS` **and to the default `core` profile** (`propose`, `explore`, `apply`, `sync`, `archive`), so `/otrix:update` is part of the default install rather than expanded-only (maintainer call on the PR).
- `docs/otrix.md` — add a `/otrix:update` row to the command table and a short "Updating a change" usage note.
...
- [#1188](https://github.com/Fission-AI/OpenSpec/issues/1188) — "Add a command to update proposal, design and task" (and stop it editing code). Delivered as `/otrix:update`, planning-artifacts-only.
...
- [#673](https://github.com/Fission-AI/OpenSpec/issues/673) — "clarify": update existing artifacts without auto-advancing the build frontier. `/otrix:update` revises in place and never creates the next artifact.
...
- [#694](https://github.com/Fission-AI/OpenSpec/issues/694), [#684](https://github.com/Fission-AI/OpenSpec/issues/684), [#618](https://github.com/Fission-AI/OpenSpec/issues/618) — "which command regenerates a document after the flow progressed / after apply?" → `/otrix:update`.
- Discussion [#1206](https://github.com/Fission-AI/OpenSpec/discussions/1206) — the official answer becomes `/otrix:update`.
...
- [#702](https://github.com/Fission-AI/OpenSpec/pull/702) `/otrix:clarify` — resolves ambiguity *within one artifact* via Q&A; a complementary upstream step. `/otrix:update` then reconciles the change's artifacts with each other.
- [#1251](https://github.com/Fission-AI/OpenSpec/pull/1251) `/otrix:review`, [#880](https://github.com/Fission-AI/OpenSpec/issues/880) — review the *implementation (code)* against the plan. `/otrix:update` is the mirror image: it keeps the *plan* coherent and never touches code.
