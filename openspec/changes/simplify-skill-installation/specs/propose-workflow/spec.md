...
- **WHEN** user invokes `/otrix:propose "add user authentication"`
...
- **WHEN** user invokes `/otrix:propose` with a name that already exists
...
- **WHEN** user invokes `/otrix:propose "feature name"`
- **THEN** the result SHALL be functionally equivalent to invoking `/otrix:new "feature-name"` followed by `/otrix:ff feature-name`
