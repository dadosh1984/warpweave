...
- Claude Code slash commands (`.claude/commands/otrix/*.md`) gain the same field — commands share the skill frontmatter contract, so the same pre-approval applies when a user runs `/otrix:*`.
