...
## Purpose
Define tool-agnostic command content and adapter contracts for generating tool-specific Warpweave command files.
...
- **THEN** `CommandContent` SHALL include:
  ...
  - `name`: human-readable name (e.g., 'Warpweave Explore')
  - `description`: brief description of command purpose
  - `category`: grouping category (e.g., 'Warpweave')
...
- **AND** file path SHALL follow pattern `.claude/commands/ww/<id>.md`

#### Scenario: Cursor adapter formatting

- **WHEN** formatting a command for Cursor
- **THEN** the adapter SHALL output YAML frontmatter with `name` as `/ww-<id>`, `id`, `category`, `description` fields
- **AND** file path SHALL follow pattern `.cursor/commands/ww-<id>.md`

#### Scenario: Windsurf adapter formatting

- **WHEN** formatting a command for Windsurf
- **THEN** the adapter SHALL output YAML frontmatter with `name`, `description`, `category`, `tags` fields
- **AND** file path SHALL follow pattern `.windsurf/workflows/ww-<id>.md`
...
- **AND** file path SHALL follow pattern `.trae/commands/ww-<id>.md`
...
- **WHEN** the adapter's file path names the command by filename (`ww-<id>`)
- **THEN** `generateCommand` SHALL rewrite `/ww:<id>` references in the body to `/ww-<id>` before formatting
- **WHEN** the adapter's file path does not name the command by filename (for example it namespaces the command under an `ww/` directory)
- **THEN** the body's `/ww:<id>` references SHALL be left unchanged

#### Scenario: Command references use the tool's own invocation prefix

- **WHEN** an adapter declares an `invocationPrefix` because its files are not invoked with a slash (Amazon Q loads `.amazonq/prompts/ww-<id>.md` into a prompt library invoked with `@`)
- **THEN** `generateCommand` SHALL rewrite `/ww:<id>` references in the body to `<prefix>ww-<id>` — for Amazon Q, `@ww-<id>` — replacing the leading slash rather than adding to it
...
- **WHEN** generating all ww commands for a tool
...
- **AND** only the frontmatter, the file path, and the spelling of `/ww:*` command references SHALL differ
