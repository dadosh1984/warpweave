...
The system SHALL resolve schemas from the project-local directory (`./warpweave/schemas/<name>/`) with highest priority when a `projectRoot` is provided.

#### Scenario: Project-local schema takes precedence over user override
- **WHEN** a schema named "my-workflow" exists at `./warpweave/schemas/my-workflow/schema.yaml`
- **AND** a schema named "my-workflow" exists at `~/.local/share/warpweave/schemas/my-workflow/schema.yaml`
...
- **WHEN** a schema named "spec-driven" exists at `./warpweave/schemas/spec-driven/schema.yaml`
...
- **WHEN** no schema named "my-workflow" exists at `./warpweave/schemas/my-workflow/`
- **AND** a schema named "my-workflow" exists at `~/.local/share/warpweave/schemas/my-workflow/schema.yaml`
- **AND** `getSchemaDir("my-workflow", projectRoot)` is called
...
- **WHEN** no schema named "spec-driven" exists at `./warpweave/schemas/spec-driven/`
- **AND** no schema named "spec-driven" exists at `~/.local/share/warpweave/schemas/spec-driven/`
- **AND** "spec-driven" is a package built-in schema
- **AND** `getSchemaDir("spec-driven", projectRoot)` is called
...
- **THEN** the system SHALL return `/path/to/project/warpweave/schemas`
...
- **WHEN** a schema named "team-flow" exists at `./warpweave/schemas/team-flow/schema.yaml`
...
- **WHEN** a schema named "my-custom" exists only at `~/.local/share/warpweave/schemas/my-custom/`
- **AND** `listSchemasWithInfo(projectRoot)` is called
...
The system SHALL use the schema field from `warpweave/config.yaml` as the default when creating new changes without explicit `--schema` flag and no planning-home default applies.
...
- **WHEN** change has `.warpweave.yaml` with `schema: bound` and config has `schema: tdd`
...
The system SHALL allow the config schema field to reference project-local schemas defined in `warpweave/schemas/`.

#### Scenario: Config references project-local schema
- **WHEN** config contains `schema: "my-workflow"` and `warpweave/schemas/my-workflow/` exists
...
- **THEN** error message includes "Fix: Edit warpweave/config.yaml and change 'schema: X' to a valid schema name"
...
- **THEN** existing changes continue to use their bound schema from `.warpweave.yaml`
