...
## Purpose
Provide a user-friendly CLI interface for viewing and modifying global Warpweave configuration settings without manually editing JSON files.
## Requirements
...
- **WHEN** user executes `warpweave config --help`
...
- **WHEN** user executes `warpweave config path`
...
- **WHEN** user executes `warpweave config get <key>` with a valid top-level key
- **THEN** print the raw value only (no labels or formatting)
- **AND** exit with code 0

#### Scenario: Get nested key with dot notation

- **WHEN** user executes `warpweave config get featureFlags.someFlag`
- **THEN** traverse the nested structure using dot notation
- **AND** print the value at that path

#### Scenario: Get non-existent key

- **WHEN** user executes `warpweave config get <key>` with a key that does not exist
- **THEN** print nothing (empty output)
- **AND** exit with code 1

#### Scenario: Get object value

- **WHEN** user executes `warpweave config get <key>` where the value is an object
...
- **WHEN** user executes `warpweave config set <key> <value>`
...
- **WHEN** user executes `warpweave config set <key> true` or `warpweave config set <key> false`
...
- **WHEN** user executes `warpweave config set <key> <value> --string`
...
- **WHEN** user executes `warpweave config set featureFlags.newFlag true`
...
- **WHEN** user executes `warpweave config unset <key>`
...
- **WHEN** user executes `warpweave config reset --all`
...
- **WHEN** user executes `warpweave config reset --all -y`
- **THEN** reset without prompting for confirmation

#### Scenario: Reset without --all flag

- **WHEN** user executes `warpweave config reset` without `--all`
...
- **WHEN** user executes `warpweave config edit`
...
The `warpweave config profile` command SHALL provide an action-first interactive flow that allows users to modify delivery and workflow settings independently.

#### Scenario: Current profile summary appears first

- **WHEN** user runs `warpweave config profile` in an interactive terminal
...
- **WHEN** user runs `warpweave config profile` interactively
...
- **WHEN** delivery selection is shown in `warpweave config profile`
...
- **WHEN** `warpweave config profile` exits with `No config changes.` inside an Warpweave project
...
- **AND** include guidance to run `warpweave update` to sync project files

#### Scenario: Apply prompt is gated on actual changes

- **WHEN** config values were changed and saved
- **AND** current directory is an Warpweave project
- **THEN** prompt `Apply changes to this project now?`
- **AND** if confirmed, run `warpweave update` for the current project
...
- **WHEN** user executes `warpweave config set someFutureKey 123`
...
- **WHEN** user executes `warpweave config set someFutureKey 123 --allow-unknown`
- **THEN** the value is saved successfully
- **AND** exit with code 0

#### Scenario: Invalid feature flag value rejected

- **WHEN** user executes `warpweave config set featureFlags.someFlag notABoolean`
...
- **WHEN** user executes `warpweave config --scope project <subcommand>`
