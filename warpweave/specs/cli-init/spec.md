...
- **AND** generate slash commands for each selected tool with a command adapter, at that adapter's own path (for example `.claude/commands/ww/<id>.md` or `.cursor/commands/ww-<id>.md`)
- **AND** create `warpweave/config.yaml` with default schema setting
...
- **WHEN** the `warpweave/` directory already exists
...
- **AND** display a getting started section naming an installed onboarding workflow (for example `/ww:propose` - Start a change)
- **AND** spell each command the way the configured tool registers it: `/ww-<id>` for tools whose command files are named `ww-<id>`, and the tool's skill invocation (`$openspec-<skill>` for Codex, `/skill:openspec-<skill>` for Kimi Code, `/openspec-<skill>` otherwise) for tools that receive no command files
...
- **GIVEN** an `warpweave/` directory already exists and at least one AI tool file is present
...
- **THEN** create 9 slash command files using the tool's command adapter:
  ...
  - `/ww:archive`
  - `/ww:bulk-archive`
- **AND** use tool-specific path conventions (e.g., `.claude/commands/ww/` for Claude)
