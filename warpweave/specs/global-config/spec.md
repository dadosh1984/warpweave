...
- **THEN** `getGlobalConfigDir()` returns `/custom/config/warpweave`
...
- **THEN** `getGlobalConfigDir()` returns `~/.config/warpweave` (expanded to absolute path)

#### Scenario: Windows platform
- **WHEN** the platform is Windows
- **AND** `%APPDATA%` is set to `C:\Users\User\AppData\Roaming`
- **THEN** `getGlobalConfigDir()` returns `C:\Users\User\AppData\Roaming\warpweave`
