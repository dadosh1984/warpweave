...
## Purpose
Define `warpweave change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `warpweave change show update-error --json`
...
- **WHEN** executing `warpweave change list`
- **THEN** scan the warpweave/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `warpweave change show update-error --requirements-only`
...
- **WHEN** executing `warpweave change validate update-error`
...
- **WHEN** executing `warpweave list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'warpweave list' is deprecated. Use 'warpweave change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `warpweave list --all`
...
- **WHEN** executing `warpweave change show` without arguments
...
- **WHEN** executing `warpweave change show` without a change name
...
- **WHEN** executing `warpweave change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `warpweave change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
