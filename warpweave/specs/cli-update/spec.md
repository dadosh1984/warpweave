...
As a developer using Warpweave, I want to update the Warpweave instructions in my project when new versions are released, so that I can benefit from improvements to AI agent instructions.
## Requirements
### Requirement: Update Behavior
The update command SHALL update Warpweave instruction files to the latest templates in a team-friendly manner.

#### Scenario: Running update command
- **WHEN** a user runs `warpweave update`
- **THEN** replace `warpweave/AGENTS.md` with the latest template
- **AND** if a root-level stub (`AGENTS.md`/`CLAUDE.md`) exists, refresh it so it points to `@/warpweave/AGENTS.md`

### Requirement: Prerequisites

The command SHALL require an existing Warpweave structure before allowing updates.

#### Scenario: Checking prerequisites

- **GIVEN** the command requires an existing `warpweave` directory (created by `warpweave init`)
- **WHEN** the `warpweave` directory does not exist
- **THEN** display error: "No Warpweave directory found. Run 'warpweave init' first."
...
- **WHEN** updating files
- **THEN** completely replace `warpweave/AGENTS.md` with the latest template
- **AND** if a root-level stub exists, update the managed block content so it keeps directing teammates to `@/warpweave/AGENTS.md`

### Requirement: Tool-Agnostic Updates
The update command SHALL refresh Warpweave-managed files in a predictable manner while respecting each team's chosen tooling.
...
- **AND** update only the Warpweave-managed sections inside existing AI tool files, leaving user-authored content untouched
- **AND** avoid creating new native-tool configuration files (slash commands, CLAUDE.md, etc.) unless they already exist

### Requirement: Core Files Always Updated
The update command SHALL always update the core Warpweave files and display an ASCII-safe success message.

#### Scenario: Successful update
- **WHEN** the update completes successfully
- **THEN** replace `warpweave/AGENTS.md` with the latest template
- **AND** if a root-level stub exists, refresh it so it still directs contributors to `@/warpweave/AGENTS.md`
...
- **THEN** refresh the Warpweave-managed portion of each file so the workflow copy matches other tools while preserving the existing single-field `description` frontmatter
...
- **AND** preserve any user customizations outside the Warpweave managed markers
...
- **THEN** refresh each file using shared templates
- **AND** include Crush-specific frontmatter with Warpweave category and tags
- **AND** ensure templates include instructions for the relevant workflow stage
...
- **AND** update only the content inside the Warpweave managed markers, leaving any unmanaged notes untouched
- **AND** skip creating missing files during update

#### Scenario: Updating slash commands for OpenCode
- **WHEN** `.opencode/commands/` contains Warpweave-managed `ww-*.md` command files for the configured profile (for example `ww-propose.md`, `ww-apply.md`, and `ww-archive.md`)
- **THEN** refresh each file using shared templates
...
- **THEN** `warpweave init` or legacy cleanup SHALL remove those files and generate replacements under `.opencode/commands/`
- **AND** `warpweave update` SHALL NOT refresh files that remain only under `.opencode/command/`

#### Scenario: Updating slash commands for Windsurf
- **WHEN** `.windsurf/workflows/` contains `openspec-proposal.md`, `openspec-apply.md`, and `openspec-archive.md`
- **THEN** refresh each file using shared templates wrapped in Warpweave markers
- **AND** ensure templates include instructions for the relevant workflow stage
...
- **AND** preserve any unmanaged content outside the Warpweave marker block
...
- **AND** update only the Warpweave-managed block between markers
- **AND** ensure templates include instructions for the relevant workflow stage
...
- **THEN** the template SHALL instruct the AI to validate the provided change ID against `warpweave list`
...
- **THEN** the template SHALL instruct the AI to identify the change ID from context or by running `warpweave list`
