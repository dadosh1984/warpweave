...
## Purpose
Define how `warpweave/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.

## Requirements
### Requirement: Load project config from warpweave/config.yaml

The system SHALL read and parse the project configuration file located at `warpweave/config.yaml` relative to the project root.

#### Scenario: Valid config file exists
- **WHEN** `warpweave/config.yaml` exists with valid YAML content
- **THEN** system parses the file and returns a ProjectConfig object

#### Scenario: Config file does not exist
- **WHEN** `warpweave/config.yaml` does not exist
- **THEN** system returns null without error

#### Scenario: Config file has invalid YAML syntax
- **WHEN** `warpweave/config.yaml` contains malformed YAML
- **THEN** system logs a warning message and returns null

#### Scenario: Config file has valid YAML but invalid schema
- **WHEN** `warpweave/config.yaml` contains valid YAML that fails Zod schema validation
...
- **WHEN** `warpweave/config.yml` exists and `warpweave/config.yaml` does not exist
- **THEN** system reads from `warpweave/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `warpweave/config.yaml` and `warpweave/config.yml` exist
- **THEN** system prefers `warpweave/config.yaml`
...
- **WHEN** config file has syntax errors and user runs `warpweave new change`
