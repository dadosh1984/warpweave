...
The system SHALL provide an `/ww:sync` skill that syncs delta specs from a change to the main specs.

#### Scenario: Sync delta specs to main specs
- **WHEN** agent executes `/ww:sync` with a change name
- **THEN** the agent reads delta specs from `warpweave/changes/<name>/specs/`
- **AND** reads corresponding main specs from `warpweave/specs/`
- **AND** reconciles main specs to match what the deltas describe

#### Scenario: Idempotent operation
- **WHEN** agent executes `/ww:sync` multiple times on the same change
- **THEN** the result is the same as running it once
- **AND** no duplicate requirements are created

#### Scenario: Change selection prompt
- **WHEN** agent executes `/ww:sync` without specifying a change
...
- **THEN** create new main spec file at `warpweave/specs/<capability>/spec.md`
- **AND** copy the delta's `## Purpose` body into it when the delta has one, matching what `warpweave archive` does
