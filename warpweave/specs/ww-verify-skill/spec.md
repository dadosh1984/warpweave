# ww-verify-skill Specification

## Purpose
Define `/ww:verify` behavior for assessing implementation completeness, correctness, and coherence against change artifacts.

## Requirements
### Requirement: Verify Skill Invocation
The system SHALL provide an `/ww:verify` skill that validates implementation against change artifacts.

#### Scenario: Verify with change name provided
- **WHEN** agent executes `/ww:verify <change-name>`
...
- **WHEN** agent executes `/ww:verify` without a change name
...
- **AND** suggests running `/ww:continue` to create tasks
...
- **WHEN** verifying completeness
- **AND** delta specs exist in `warpweave/changes/<name>/specs/`
