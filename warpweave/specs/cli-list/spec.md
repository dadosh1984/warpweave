...
- **THEN** scan the `warpweave/changes/` directory for change directories
- **AND** exclude the `archive/` subdirectory from results
- **AND** parse each change's `tasks.md` file to count task completion

#### Scenario: Scanning for specs
- **WHEN** `warpweave list --specs` is executed
- **THEN** scan the `warpweave/specs/` directory for capabilities
...
- **WHEN** `warpweave/changes/` directory doesn't exist
