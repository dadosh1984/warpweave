...
The CLI SHALL provide an `warpweave schema fork <source> [name]` command that copies an existing schema to the project's `warpweave/schemas/` directory.
...
- **AND** copies all files to `warpweave/schemas/my-custom/`
...
- **THEN** system copies to `warpweave/schemas/spec-driven-custom/`
...
- **WHEN** user runs `warpweave schema fork spec-driven my-custom` and `warpweave/schemas/my-custom/` exists
