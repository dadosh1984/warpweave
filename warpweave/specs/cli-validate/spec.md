...
## Purpose
Define `warpweave validate` behavior for validating changes and specs with actionable remediation guidance and structured output.
...
- **THEN** show error "No deltas found" with guidance:
  ...
  - Remind authors that files must live under `warpweave/changes/{id}/specs/<capability>/spec.md`
  - Include an explicit note: "Spec delta files cannot start with titles before the operation headers"
  - Suggest running `warpweave change show {id} --json --deltas-only` for debugging
...
- **THEN** include expected header names and a minimal skeleton:
  ...
  - Mention the quick-reference section in `warpweave/AGENTS.md` as the authoritative template
...
- **THEN** emit an error explaining that `### Requirement:` lines must be followed by narrative text before any `#### Scenario:` headers
  ...
  - Reference the pre-validation checklist in `warpweave/AGENTS.md`
...
- Source file path (`warpweave/changes/{id}/proposal.md`, `.../specs/{cap}/spec.md`)
...
- **THEN** print "Next steps" with 2-3 targeted bullets and suggest `warpweave change show <id> --json --deltas-only`
...
Validation run inside `warpweave archive` SHALL NOT report these issues, because archive enforces the same check when it applies the deltas.

#### Scenario: MODIFIED omits an existing scenario

- **GIVEN** the main spec's requirement has scenarios "A" and "B"
- **WHEN** a change MODIFIES that requirement with only scenario "A" and `warpweave validate <change>` runs
...
- **WHEN** `warpweave validate <change>` runs
...
- **WHEN** executing `warpweave validate` without arguments
...
- **WHEN** executing `warpweave validate <item-name>`
...
- **WHEN** executing `warpweave validate --all`
- **THEN** validate all changes in warpweave/changes/ (excluding archive)
- **AND** validate all specs in warpweave/specs/
...
- **THEN** include all change proposals under `warpweave/changes/`
- **AND** exclude the `warpweave/changes/archive/` directory

- **WHEN** validating with `--specs`
- **THEN** include all specs that have a `spec.md` under `warpweave/specs/<id>/spec.md`

#### Scenario: Validate all changes

- **WHEN** executing `warpweave validate --changes`
- **THEN** validate all changes in warpweave/changes/ (excluding archive)
...
- **WHEN** executing `warpweave validate --specs`
- **THEN** validate all specs in warpweave/specs/
...
- **WHEN** executing `warpweave validate --all --strict`
...
- **WHEN** executing `warpweave validate --all --json`
...
- **WHEN** executing `warpweave validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `warpweave change validate` / `warpweave spec validate`
...
- **WHEN** executing `warpweave validate --type change <item>`
- **THEN** treat `<item>` as a change ID and validate it (skipping auto-detection)

- **WHEN** executing `warpweave validate --type spec <item>`
...
- **WHEN** `warpweave validate` is executed with `--no-interactive` or with environment `OPEN_SPEC_INTERACTIVE=0`
...
- **WHEN** running `warpweave validate <change-id>`
