# WW Archive Skill Spec

## Purpose

Define the expected behavior for the `/ww:archive` skill, including readiness checks, spec sync prompting, archive execution, and user-facing output.

## Requirements

### Requirement: WW Archive Skill

The system SHALL provide an `/ww:archive` skill that archives completed changes in the experimental workflow.

#### Scenario: Archive a change with all artifacts complete

- **WHEN** agent executes `/ww:archive` with a change name
- **AND** all artifacts in the schema are complete
- **AND** all tasks are complete
- **THEN** the agent moves the change to `warpweave/changes/archive/<target-name>/`
- **AND** displays success message with archived location

#### Scenario: Change selection prompt

- **WHEN** agent executes `/ww:archive` without specifying a change
...
- **AND** if user confirms, execute `/ww:sync` logic inline and wait for it to complete
...
- **AND** preserve `.warpweave.yaml` file in archived change
...
- **THEN** display summary:
  - Specs synced (from `/ww:sync` output)
