...
## Purpose
Provide programmatic utilities for creating and validating Warpweave change directories.
## Requirements
...
- **THEN** the system creates `warpweave/changes/add-auth/` directory

#### Scenario: Duplicate change rejected
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `warpweave/changes/add-auth/` already exists
- **THEN** the system throws an error indicating the change already exists

#### Scenario: Creates parent directories if needed
- **WHEN** `createChange(projectRoot, 'add-auth')` is called and `warpweave/changes/` does not exist
