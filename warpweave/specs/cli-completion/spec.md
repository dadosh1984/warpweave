...
## Purpose
Provide shell completion scripts for the Warpweave CLI, enabling tab-completion for commands, flags, and dynamic values (change IDs, spec IDs) across multiple shells. Supports Zsh, Bash, Fish, and PowerShell.
## Requirements
...
- **WHEN** user executes `warpweave completion --help`
...
- **WHEN** user executes `warpweave completion generate zsh`
...
- **WHEN** user executes `warpweave completion generate bash`
...
- **AND** support dynamic completion for change and spec IDs via `warpweave __complete`

#### Scenario: Generating Fish completion

- **WHEN** user executes `warpweave completion generate fish`
...
- **WHEN** user executes `warpweave completion generate powershell`
...
- **THEN** discover active changes from `warpweave/changes/` directory
- **AND** exclude archived changes in `warpweave/changes/archive/`
- **AND** return change IDs as completion suggestions
- **AND** only provide suggestions when inside an Warpweave-enabled project

#### Scenario: Completing spec IDs

- **WHEN** completing arguments for commands that accept spec names (show, validate)
- **THEN** discover specs from `warpweave/specs/` directory
...
- **WHEN** user requests completions outside an Warpweave project
...
- **WHEN** user executes `warpweave completion install zsh`
...
- **AND** write completion script to `~/.oh-my-zsh/custom/completions/_warpweave`
...
- **WHEN** user executes `warpweave completion install zsh` and Oh My Zsh is not detected
- **THEN** create completions directory at `~/.zsh/completions/` if it doesn't exist
- **AND** write completion script to `~/.zsh/completions/_warpweave`
...
- **WHEN** user executes `warpweave completion install bash`
...
- **WHEN** user executes `warpweave completion install fish`
...
- **WHEN** user executes `warpweave completion install powershell`
...
- **WHEN** user executes `warpweave completion install` without specifying a shell
...
- **WHEN** user executes `warpweave completion uninstall zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_warpweave` if Oh My Zsh is detected
- **AND** remove `~/.zsh/completions/_warpweave` if standard Zsh setup is detected
- **AND** remove fpath modifications from `~/.zshrc` using marker-based removal
- **AND** display success message

#### Scenario: Uninstalling Bash completion

- **WHEN** user executes `warpweave completion uninstall bash`
...
- **WHEN** user executes `warpweave completion uninstall fish`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `warpweave completion uninstall powershell`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `warpweave completion uninstall` without specifying a shell
...
- **AND** implement methods:
  ...
  - `isOpenSpecProject(): boolean` - Checks if current directory is Warpweave-enabled
...
- **WHEN** `warpweave completion install` cannot detect current shell
- **THEN** display error: "Could not auto-detect shell. Please specify shell explicitly."
- **AND** display usage hint: "Usage: warpweave completion <operation> [shell]"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `warpweave completion generate zsh > /path/to/_warpweave`
