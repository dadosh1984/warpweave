...
async function hasWritableModeAndAccess(targetPath: string): Promise<boolean> {
  try {
    ...
    // POSIX root can often write despite mode bits, but Warpweave should respect
...
export class FileSystemUtils {
  ...
  static async ensureWritePermissions(dirPath: string): Promise<boolean> {
    try {
      ...
      const testFile = path.join(dirPath, '.warpweave-test-' + Date.now() + '-' + Math.random().toString(36).slice(2));
