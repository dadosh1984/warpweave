...
/**
 * Returns the ids of active changes: every directory under warpweave/changes/
 * (legacy installs: warpweave/changes/), except the archive and hidden directories.
 *
 * A change is resolved by its directory alone - the same rule `list`,
 * `status`, `instructions` and `validate` use (`getAvailableChanges`).
 * Requiring proposal.md here made `warpweave show` and shell completion miss
 * changes those commands resolve: `warpweave new change <name>` scaffolds only
 * `.warpweave.yaml`, and a custom schema need not define a proposal artifact at
 * all (#1161).
 */
export async function getActiveChangeIds(root: string = process.cwd()): Promise<string[]> {
  const changesPath = path.join(root, resolvePlanningDirName(root), 'changes');
  try {
...
export async function getSpecIds(root: string = process.cwd()): Promise<string[]> {
  const specsPath = path.join(root, resolvePlanningDirName(root), 'specs');
 ...
 * warpweave/changes/archive/ except hidden directories.
 *
...
export async function getArchivedChangeIds(root: string = process.cwd()): Promise<string[]> {
  const archivePath = path.join(root, resolvePlanningDirName(root), 'changes', 'archive');
  try {
