 ...
 *
 * Command bodies are authored with `/ww:<id>` references. Tools whose command
 * files are invoked by filename register `/ww-<id>` instead, and Amazon Q
 * surfaces them in its prompt library as `@ww-<id>`, so the body is rewritten
