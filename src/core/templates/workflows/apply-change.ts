...
1. **Select the change**
   ...
   Always announce: "Using change: <name>" and how to override (e.g., \`/ww:apply <other>\`).
...
**Input**: Optionally specify a change name (e.g., \`/ww:apply add-auth\`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If \`state: "blocked"\` (missing artifacts): show message, suggest using \`/ww:continue\` (if it is not installed, run \`warpweave status --change "<name>" --json\` to see the next artifact and \`warpweave instructions <artifact-id> --change "<name>" --json\` for how to create it)
   - If \`state: "all_done"\`: congratulate, suggest archive
   - Otherwise: proceed to implementation
