...
export function getOpsxNewCommandTemplate(): CommandTemplate {
  return {
    name: 'OTRIX: New',
    description: 'Start a new change using the experimental artifact workflow (OTRIX)',
    category: 'Workflow',
...
**Input**: The argument after \`/otrix:new\` is the change name (kebab-case), OR a description of what the user wants to build.
...
- Current status (0/N artifacts complete)
- The template for the first artifact
- Prompt: "Ready to create the first artifact? Run \`/otrix:continue\` or just describe what this change is about and I'll draft it."

**Guardrails**
- Do NOT create any artifacts yet - just show the instructions
- Do NOT advance beyond showing the first artifact template
- If the name is invalid (not kebab-case), ask for a valid name
- If a change with that name already exists, suggest using \`/otrix:continue\` instead
- Pass --schema if using a non-default workflow`
  };
}
