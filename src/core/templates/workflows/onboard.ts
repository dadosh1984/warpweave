...
function getOnboardInstructions(): string {
  return `Guide the user through their first complete Spectrix workflow cycle. This is a teaching experience—you'll do real work in their codebase while explaining each step.
...
> Spectrix CLI is not installed. Install it first, then come back to \`/otrix:onboard\`.
...
- \`/otrix:continue <name>\` - Resume artifact creation (if installed; otherwise \`spectrix status --change "<name>" --json\` shows the next artifact)
...
export function getOpsxOnboardCommandTemplate(): CommandTemplate {
  return {
    name: 'OTRIX: Onboard',
    description: 'Guided onboarding - walk through a complete Spectrix workflow cycle with narration',
    category: 'Workflow',
