...
export class ValidateCommand {
   ...
   * same rule `warpweave status`/`instructions` use (`getAvailableChanges`) —
   ...
   */
  private async listChangeIds(root: ResolvedWarpweaveRoot): Promise<string[]> {
    const ids = await getAvailableChanges(root.path, root.changesDir);
    return ids.sort();
  }

  private async runInteractiveSelector(root: ResolvedWarpweaveRoot, opts: { strict: boolean; json: boolean; concurrency?: string }): Promise<void> {
  ...
  private printNonInteractiveHint(root: ResolvedWarpweaveRoot): void {
    ...
    console.error(`  ${withStoreFlag(root, 'warpweave validate --all')}`);
    console.error(`  ${withStoreFlag(root, 'warpweave validate --changes')}`);
    console.error(`  ${withStoreFlag(root, 'warpweave validate --specs')}`);
    console.error(`  ${withStoreFlag(root, 'warpweave validate <item-name>')}`);
    console.error('Or run in an interactive terminal.');
  }

  private async validateDirectItem(root: ResolvedWarpweaveRoot, itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    const [changes, specs] = await Promise.all([this.listChangeIds(root), getSpecIds(root.path)]);
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      if (isStoreSelectedRoot(root)) {
        ...
        console.error('Pass --type change|spec, or use: warpweave change validate / warpweave spec validate');
      }
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(root: ResolvedWarpweaveRoot, type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
  ...
  private printReport(type: ItemType, id: string, report: { valid: boolean; issues: any[] }, durationMs: number, json: boolean, root: ResolvedWarpweaveRoot): void {
    if (json) {
  ...
  private printNextSteps(type: ItemType, id: string, root: ResolvedWarpweaveRoot, issues: Array<{ message: string }> = []): void {
    ...
    if (type === 'change' && conflictIssue) {
      ...
      bullets.push('- skip_specs is only honored when .warpweave.yaml is valid change metadata (schema: <name> naming a known schema is required)');
    } else if (type === 'change' && invalidMarkerIssue) {
      bullets.push('- Fix .warpweave.yaml so the skip_specs marker can be honored (schema: <name> naming a known schema is required)');
      bullets.push('- Or remove skip_specs from .warpweave.yaml and add delta specs instead');
    } else if (type === 'change') {
      bullets.push('- Ensure change has deltas in specs/: use headers ## ADDED/MODIFIED/REMOVED/RENAMED Requirements');
      bullets.push('- Each requirement MUST include at least one #### Scenario: block');
      bullets.push(`- Debug parsed deltas: ${withStoreFlag(root, `warpweave show ${id} --json --deltas-only`)}`);
    } else {
  ...
  private async runBulkValidation(root: ResolvedWarpweaveRoot, scope: { changes: boolean; specs: boolean }, opts: { strict: boolean; json: boolean; concurrency?: string; noInteractive?: boolean }): Promise<void> {
    ...
    const concurrency = normalizeConcurrency(opts.concurrency) ?? normalizeConcurrency(process.env.WARPWEAVE_CONCURRENCY) ?? DEFAULT_CONCURRENCY;
    const validator = new Validator(opts.strict);
    ...
    if (opts.json) {
      ...
      if (firstFailure) {
        ...
        console.log(
          `Details: warpweave validate ${firstFailure.id} --type ${firstFailure.type}${storeFlag}`
        );
      }
    }
