...
/**
 * Command for managing shell completions for Spectrix CLI
 */
export class CompletionCommand {
  ...
  private resolveShellOrExit(shell: string | undefined, operationName: string): SupportedShell | null {
    ...
    if (!normalizedShell) {
      ...
      console.error(`Usage: spectrix completion ${operationName} [shell]`);
  ...
  private async uninstallForShell(shell: SupportedShell, skipConfirmation: boolean): Promise<void> {
    ...
    if (!skipConfirmation) {
      ...
      const confirmed = await confirm({
        message: `Remove Spectrix configuration from ${configPath}?`,
        default: false,
      });
