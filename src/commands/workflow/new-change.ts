 ...
 * resolved Warpweave root. `--store <id>` selects a registered store's
...
function assertRemovedOptionsAbsent(options: NewChangeOptions): void {
  if (options.initiative !== undefined) {
    throw new RootSelectionError(
      '--initiative is no longer supported. Normal changes no longer attach to initiatives; --store <id> selects the Warpweave root.',
  ...
  if (options.areas !== undefined) {
    throw new RootSelectionError(
      '--areas is no longer supported. Workspace affected areas are not part of the normal Warpweave root path.',
...
function printCreatedChangeHuman(
  ...
  console.log(`Next: ${withStoreFlag(root, `warpweave status --change ${payload.change.id}`)}`);
}
  ...
  try {
    ...
    const payload: NewChangeOutput = {
      change: {
        ...
        metadataPath: path.join(result.changeDir, '.warpweave.yaml'),
